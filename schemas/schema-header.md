...
- **Skill** defines HOW to do it: a container for named `operations`, `rules`, and `errors`. Activities and workflows compose behavior by listing `skill-id::operation-name` references in their `operations:` arrays.
- **Condition** defines conditional expressions used by transitions, decisions, checkpoints, and loops for control flow.
