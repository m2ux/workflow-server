...
2. For any operation in the bundle whose body declares a `resources[]` array, call `get_resource { session_index, resource_id }` for each resource id. This applies equally to techniques inlined under `step_techniques` (below): resources are never inlined — load each referenced resource on demand with `get_resource`.
