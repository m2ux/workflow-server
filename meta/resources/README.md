...
|-------------|----------|---------|
| `bootstrap-protocol` | [Bootstrap Protocol](./bootstrap-protocol.md) | Pre-session stub served by `discover` — schema fetch, always bind `repo` on `start_session`, `get_workflow`; ongoing delivery policy lives in the operations bundle. |
