...
- Estimate: expected agentic time — adjust template defaults to the work's complexity. It stays an estimate for the life of the table: nothing writes a measured time back into it, so a reader takes the column as what the work was expected to cost and reads what it did cost from the run's own usage record.
