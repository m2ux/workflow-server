...
The operation a step binds, named by `step.technique` — either a bare string (`group::operation`, a single-file id, or — per the `activity-group-shorthand` rule — a bare op id resolved against the activity's same-named group), or, when the step supplies deviations, the `name` field of a structured `step.technique` object `{ name, inputs, outputs }`. Its composed `inputs[]`/`outputs[]` signature is the binding contract.
...
5. Land outputs. For each declared output id `O`, take the produced value and commit it to `{variable_bag}` under `O` (or under the remapped bag name when `step.technique.outputs` maps `O` to a different name), forming `{landed_outputs}`. Nested-object outputs land whole, so a dotted-path read downstream resolves against the landed object.
