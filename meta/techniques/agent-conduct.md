...
Most workflow artifacts — the per-activity planning documents — live in the TARGET component repository (your CWD is `target_path`) under the work package's planning subfolder (`.engineering/artifacts/planning/<work-package-subfolder>/`, exactly ONE per work package), provided by `planning_folder_path`; write them there, NEVER to the bare `planning/` root, and NEVER resolve the path relative to an arbitrary working directory other than `target_path`. A few dedicated artifact classes are deliberate exceptions, living in their own conventional directory set by the technique that owns them — Architecture Decision Records under `.engineering/artifacts/adr/` (create-adr's `adr_dir`) and codebase-comprehension docs under `.engineering/artifacts/comprehension/` (build-comprehension's `comprehension_dir`) — and are written there rather than the planning subfolder. Each artifact filename carries the server-provided `artifactPrefix` for its activity (e.g. `09-code-review.md`). This path is relative to the target repo and resolves correctly even when the target component is the workflow-server repository itself. A logical artifact (a given bare filename) has exactly ONE numbered instance — created by its first activity; any later activity that writes the same bare filename UPDATES that instance in place, preserving its original number, never minting a new prefix (only token-templated names like `strategic-review-{n}.md` create a numbered series).
