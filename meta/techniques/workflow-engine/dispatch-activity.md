...
Current variable bag for prompt substitution and for the worker's bound technique inputs. The bag is the single source of truth (including `{target_repo}`); do not invent a parallel prior-state channel in the prompt.
