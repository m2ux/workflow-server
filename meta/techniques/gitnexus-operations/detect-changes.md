...
1. Call `gitnexus_detect_changes {diff_scope, repo_name}` to produce the `{change_report}` (changed symbols, changed files, affected flows, risk level). If the index is out of date, run `npx gitnexus analyze`, then retry.
