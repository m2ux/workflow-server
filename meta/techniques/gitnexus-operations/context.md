...
1. Call `gitnexus_context {name}` to assemble the `context-report` — incoming calls, outgoing calls, and process membership.
...
2. Read the `context-report`'s caller fan-out as a blast-radius signal: many callers and broad process participation → the symbol is path-committing; an isolated symbol is low-risk to touch.
