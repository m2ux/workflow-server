...
1. If cargo nextest is configured (`.config/nextest.toml` present in the project): `SKIP_WASM_BUILD=1 CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-4} RUST_TEST_THREADS=${RUST_TEST_THREADS:-4} nice -n 19 cargo nextest run {build_scope} {features} --test-threads=${RUST_TEST_THREADS:-4} {test_filter}`  
...
2. Otherwise: `SKIP_WASM_BUILD=1 CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-4} RUST_TEST_THREADS=${RUST_TEST_THREADS:-4} nice -n 19 cargo test {build_scope} {features} {test_filter} -- --test-threads=${RUST_TEST_THREADS:-4}`
