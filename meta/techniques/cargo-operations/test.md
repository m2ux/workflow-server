...
The command prefix this operation carries, capping test threads alongside the compile caps.

#### default

`SKIP_WASM_BUILD=1 CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-4} RUST_TEST_THREADS=${RUST_TEST_THREADS:-4} nice -n 19`
...
1. If cargo nextest is configured (`.config/nextest.toml` present in the project): `{build_budget} cargo nextest run {build_scope} {features} --test-threads=${RUST_TEST_THREADS:-4} {test_filter}`  
...
2. Otherwise: `{build_budget} cargo test {build_scope} {features} {test_filter} -- --test-threads=${RUST_TEST_THREADS:-4}`
