...
array of `{ file, rule, detail, fixed }` entries — one per detected violation, where `rule` names the discipline breached (`no-guide`, `single-source-and-link`, `state-once-per-artifact`, `omit-null-sections`, `exception-only-reporting`, `line-budget`, or `writing-register`) and `fixed` records whether the in-place fix was applied.
