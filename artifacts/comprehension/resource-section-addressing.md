...
| Q9 | Which of the three broken-heading links is in scope for this package, given only one targets a `resources/` file? | Resolved | All three, by user decision at elicitation; the fourth (hidden) link is deferred. Measurement also reclassified them: they are not three unrelated heading breaks but *two* links into fenced template bodies plus one heading that was never written | [Elicitation measurements](#requirements-elicitation-measurements--the-fenced-template-link-class--2026-07-27) |
| Q10 | Should the anchor guard gain a third `reason` (`ambiguous-anchor`) so first-match over-delivery becomes representable? | Resolved | **Yes, and it is the sole mechanism.** Decided by the user at elicitation: the guard gains `ambiguous-anchor` and the over-delivery class becomes merge-blocking, while `extractMarkdownSection` keeps GitHub `-N` semantics and does not throw on a bare slug — a resolver throw would fail a link both GitHub and the guard accept, relocating Defect 1 rather than fixing it | [Elicitation measurements](#requirements-elicitation-measurements--the-fenced-template-link-class--2026-07-27) |
...
That question was put to the user at requirements elicitation and answered: **a GitHub-resolvable address is worth more.** Path-scoped addressing was descoped from #141 and deferred. The measurements that decided it are below.

### Requirements-elicitation measurements — the fenced-template link class — 2026-07-27

Three probes taken during elicitation, each bearing on whether hierarchical addressing had a live consumer.

**1. The issue's strongest hierarchy case does not need hierarchy.** `substrate-node-security-audit/resources/audit-prompt-template.md` points **8** sweeps at the same `static-analysis-patterns.md#grep-patterns` (lines 97, 103, 106, 109, 112, 115, 121, 124), each wanting one subsection — the issue's clearest "consumer wants a leaf" argument. But every `###` leaf under `## Grep Patterns` (13 of them: `panic-paths`, `unsafe-code`, `cryptographic-weaknesses`, `type-casts`, `feature-flags`, `resource-consumption`, `database-connection-security`, `information-leaks--credentials`, `information-leaks--topology`, `file-io-safety`, `serialization-pre-allocation-mismatch`, `mock-data-source-toggle`, `empty-vs-absent-query-pattern`) has a **file-unique slug**. The near-collisions with the `## Mechanical Checks` section resolve apart because those headings carry a `Check N:` prefix. So all 8 links can be repointed to unique **flat** anchors today, with no code change, and they stay GitHub-clickable. The same holds for the 3 links targeting `#mechanical-checks`.

**2. The second duplicate-slug file has no live victim.** `cicd-pipeline-security-audit/resources/intermediate-artifact-schemas.md` carries `field-descriptions` ×5, but all 12 live referencers target other, unique anchors (`#workflow-inventory`, `#scanner-assignments`, `#merged-findings`, `#reconciliation`, `#verification-report`). Zero live refs reach a duplicate.

**3. The only genuine hierarchy case has no anchored referencer.** `injection-pattern-catalog.md` is the one file where a path ref beats positional `-N` on meaning and stability — 7× `### Grep Patterns` and 7× `### Description` under `## P1 — Expression Injection` … `## P7`, so `-N` addressing yields `#grep-patterns-3` where a path yields `#p4--excessive-permissions/grep-patterns`. But its only consumer (`techniques/scan-injection-patterns/load-patterns.md:14`) loads the whole 326-line file with **no anchor at all**, and repointing it is a referencer refactor the issue's own Out-of-scope section excludes. Nothing is broken today; the value is entirely prospective.

Together these leave hierarchy with no in-scope consumer, which is what decided the descope.

**A previously unnamed defect class: links into fenced template bodies.** Three of the four known anchor breaks share one shape — the target heading exists, but only *inside* a fenced template, so GitHub renders no anchor for it and both the guard and the (fence-aware) resolver correctly refuse it:

| Link | Target heading | Fence | Real heading outside the fence |
|---|---|---|---|
| `review-assumptions/interview.md → assumptions-review.md#open-assumptions` | `## Open Assumptions`, line 70 | 46–91 | `## Assumptions Log Template`, line 41 |
| `review-test-suite.md → test-suite-review.md#test-suite-review-report-template` | `# Test Suite Review Report`, line 51 | 50–147 | `## Report Template`, line 46 |
| `architecture-summary.md:214 → work-package-plan.md#dependencies--risks` (hidden) | `## Dependencies & Risks`, `wp-plan.md` line 65 | 16–77 | — (also the wrong filename; the file is `wp-plan.md`) |

The fourth, `workflow-orchestrator.md → dispatch-activity.md#accumulate-trace-tokens`, is different: no such heading exists anywhere. The accumulate half of that contract lives in the Protocol body (line 44) and inside the `resolve-trace-at-close-out` rule (line 56), so the fix is a content decision — write the rule heading, or repoint the link — not a typo correction.

The class matters because it is generated by a corpus-wide convention (guide resources wrap their template in a fence, and sibling documents link to headings inside it), and because **AC 6's fence hardening makes the guard see more of it**. How many additional links a CommonMark-correct tracker exposes is unmeasured, and since a strengthened guard cannot merge red, that count gates the criterion. Note the asymmetry with the resolver-side finding above: 0 *resource* files parse differently under a correct tracker, but the guard scans 137 links in non-resource files where the convention is concentrated.

**The ambiguity error collides with GitHub semantics.** The descope left "an ambiguous flat slug fails loudly" as the package's second half, and tracing it against the adopted semantics exposes a tension the issue does not anticipate: once `-N` dedupe is honoured, a bare `#slug` **legally** denotes the first of N — that is what GitHub does and what the guard already approves. A runtime-only throw therefore takes a guard-valid, GitHub-valid link and fails it at runtime, which is precisely the defect class Defect 1 exists to eliminate. Under `-N` semantics the 7 live `prism-evaluate` refs are not ambiguous but simply **wrong**: they name the H1 and wanted the H2, and a human clicking them on GitHub also lands on the H1. No resolver can infer that intent, which is the argument for locating the check in the guard (Q10's `ambiguous-anchor` reason) rather than in `extractMarkdownSection`.

**Resolution.** The user chose the guard-only realisation: `check:anchors` gains `ambiguous-anchor` and the class becomes merge-blocking, while `extractMarkdownSection` keeps GitHub `-N` semantics exactly. Two consequences worth carrying forward. First, the two-specification problem this artifact opens with does **not** end here — it narrows: the guard and the runtime will share one slugger and be tied together by a differential check, but the guard also keeps a *stricter* rule (bare-slug-over-duplicates is an error) that the runtime deliberately does not enforce. That divergence is intentional and directional — the guard is stricter than the resolver, never more permissive — which is the safe orientation, and the inverse of today's defect. Second, `BrokenAnchor` gains its third reason, so a *resolves-but-to-the-wrong-heading* outcome becomes representable for the first time; the Core Types entry noting that gap is now historical.

The 7 refs, for the record — `evaluation-plan-template.md#evaluation-plan-template` (`plan-evaluation/write-evaluation-plan.md:28,48`), `mitigation-plan-template.md#mitigation-plan-template` (`resolve-findings/compile-plan.md:18`, `resolve-findings/TECHNIQUE.md:32`), `evaluation-report-template.md#evaluation-report-template` (`resolve-findings/TECHNIQUE.md:14`, `compose-evaluation-report/TECHNIQUE.md:32`, `compose-evaluation-report/compose-report.md:18`). For `evaluation-report-template.md` the H1 sits at line 9 and the intended H2 at line 17 with its fenced body running 19–92, so the GitHub dedupe table is `{evaluation-report-template, evaluation-report-template-1}` and `-1` binds the intended section.
