...
| 9 | Should S2 warn or block, given no fidelity check has ever raised an error? | Open — stakeholder | Not a comprehension question. The stance is uniform today (advisory), and per-call blast radius is one tool call, so blocking is recoverable but unprecedented on this surface. Belongs to elicitation | — |
