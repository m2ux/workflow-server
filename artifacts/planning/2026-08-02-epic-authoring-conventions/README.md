...
7. **`## Tracking`** — the sentence "Each work item is delivered as its own pull request when picked up", then a checkbox per W-item with its gate stated inline ("gated on #402 W1"), then the consolidation line: "**Consolidates #X (W1) and #Y (W2, W3)**; both bodies are captured verbatim in the planning folder."
...
- **One pull request per work item**, opened when picked up — never a long-lived stub PR. (Origin of this rule: four stub PRs — #373, #375, #376, #377 — sat with only a branch-opening commit and were converted back into work items on 2026-08-02.)
- **A work item never lives outside its epic.** The rule above once read "one PR *or spawned issue* per work item", and #428 took the second option: W3 of the artifact-audience epic was spawned as its own issue on 2026-08-03 so a per-artifact review could settle the scope. The review was worth doing and its result is good, but the issue then read as unhomed on the tracker for three days, which is the one thing the epic tier exists to prevent. Folded back on 2026-08-06. If a scoping question has to be answered before a work item can be stated, answer it in the planning folder and write the answer into the W item — the epic is the home, and a pull request is the only thing that leaves it.
