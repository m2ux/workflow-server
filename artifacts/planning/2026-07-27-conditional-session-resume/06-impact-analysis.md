...
**Mechanism note (bounds what G4 can achieve).** `action: set` is agent-executed, not server-applied: no server path ingests `variables_changed`, and step `when` / `condition` are honoured by the executing agent, not evaluated by the engine. So `record-match` deriving `has_saved_state` fixes the self-referential gate within the executing agent's context, but the value still never enters the durable bag. Only the `resume-session` checkpoint's `setVariable` effect on `is_resuming` is engine-applied — which is what `initialize-session` reads, so the resume path holds. The schema also flags `set` for removal at the next workflow-schema major (#166 B7/B12).
