...
| A-4 | Rule Scope | L | audit | The conflicting rule is replaced in place and stays activity-local, rather than being deleted or promoted to `workflow.yaml` `rules.workflow` | The constraint governs one activity's step sequence only | ✅ Validated — structural inventory records `rules.activity` unset and one `discover-session`-local rule; design-principles #6 | None; spec unchanged |
