...
2. **`decisions[]` and `transitions[]` stay activity-level.** They route to *other activities* and are evaluated by the **orchestrator** at the activity boundary (`getValidTransitions` in `workflow-loader.ts:257`). Pulling them into the worker-facing `steps[]` would cross the orchestrator/worker boundary. They are out of scope for the in-activity sequence.
