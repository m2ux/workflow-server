...
Each instruction sheet in this system tells the assistant how to do one job, and a sheet will sometimes say, partway through, to go and do what another sheet says and hand it a couple of values. Today the server that hands out these sheets treats those sentences as ordinary text: it does not go and fetch the other sheet, it does not check that the values being passed are the ones that sheet asks for, and it does not send that sheet along with the first. This work teaches the server to read those sentences properly — to find the sheet being named, send it too, and compare the values handed over against what the receiving sheet says it needs. Alongside that it adds an automatic check that finds every one of these hand-offs and lists the ones that do not add up, and it replaces a group of sheets that did nothing but wrap a single tool call with the tool itself, so there is one less layer to keep in step.

The practical gain is that a whole category of quiet breakage stops being quiet. Renaming a sheet, or changing the values it needs, currently reaches the sheets that call it with no warning at all, and examples of that drift are already sitting in the system; after this work such a change fails a check instead. A sheet named in the middle of an instruction now arrives with that instruction, so the assistant follows what was written rather than improvising around a missing file. The written design rules, the automatic checks and the running server are brought into agreement in the same pass, which matters because three of them currently disagree about whether these hand-offs are even allowed. Two limits are worth stating plainly: the check produces a worklist of the remaining hand-offs rather than repairing them, which is deliberate and scheduled as separate follow-on work; and the rules for how these hand-offs are counted are published alongside the count, because measuring this area has repeatedly produced different numbers from definitions that read identically.
