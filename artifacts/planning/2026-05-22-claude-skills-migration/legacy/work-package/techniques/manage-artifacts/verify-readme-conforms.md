...
Verify the planning folder's README.md still matches the [readme](../../resources/readme/SKILL.md) template structure (header block, executive summary, problem/solution overview, progress table, links). Catches template drift that would otherwise only be flagged late in the workflow by a human reviewer.
...
1. Read `{planning_folder_path}/README.md` and load [readme](../../resources/readme/SKILL.md) (the README template) via `get_resource`.
