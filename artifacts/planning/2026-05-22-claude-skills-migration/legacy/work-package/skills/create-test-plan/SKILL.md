...
- Use attached resource 11 (test-plan) for test plan template and guidance
...
- Use `gitnexus_query({query: <concept>})` to find existing test patterns for related concepts and `gitnexus_context({name: <symbol>})` to identify error-path callees as edge-case test candidates. See resource 27.
...
- [test-plan](skill:legacy/work-package/resources/test-plan)
