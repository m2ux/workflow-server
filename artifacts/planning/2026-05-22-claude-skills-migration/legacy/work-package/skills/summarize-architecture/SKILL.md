...
- Use `gitnexus_detect_changes()` to bound the scope of diagrams to the affected processes and symbols. See resource 27.
...
- Use Mermaid syntax
- Source package-diagram structure from `gitnexus://repo/{name}/clusters` (functional areas with cohesion scores) and `gitnexus://repo/{name}/cluster/{name}` (area members) when the codebase is indexed.
...
- Source sequence-diagram structure from `gitnexus://repo/{name}/process/{name}` (step-by-step execution traces) when the codebase is indexed.
...
- [architecture-summary](skill:legacy/work-package/resources/architecture-summary)
