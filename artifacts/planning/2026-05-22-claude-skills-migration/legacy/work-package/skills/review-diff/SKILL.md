...
- When a block centres on a graph-resolvable symbol, enrich the Block Rationale with caller/callee/process context from `gitnexus_context({name: <symbol>})` so the reviewer understands why the diff matters and which execution flows it touches. See resource 27.
...
- [manual-diff-review](skill:legacy/work-package/resources/manual-diff-review)
