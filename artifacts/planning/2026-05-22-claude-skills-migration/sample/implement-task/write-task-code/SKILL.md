...
- A context summary exists in agent state (output of
  [understand-task-context](skill:implement-task/understand-task-context)).
- If the codebase is GitNexus-indexed, an upstream impact report has
  been obtained via [impact](skill:gitnexus/impact) and reviewed;
...
- **TDD for Rust.** When the change is in a Rust crate,
  [tdd-design-rust](skill:testing/tdd-design-rust) is applied —
...
[verify-task-locally](skill:implement-task/verify-task-locally).
