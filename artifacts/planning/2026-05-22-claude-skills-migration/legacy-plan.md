...
- `## Inputs`, `## Output`, `## Errors`, and `## Rules` ALL use the **named-subsection form** — each entry is `### <name>` with a body underneath. This mirrors the protocol-technique convention exemplified by [analyze-implementation/SKILL.md](legacy/work-package/techniques/analyze-implementation/SKILL.md). Bodies can be a single short paragraph (Inputs/Output) or multi-paragraph (Errors/Rules) — the form is uniform either way. Bullets in those four sections are non-canonical and break the server's TOON projection.
