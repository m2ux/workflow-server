...
- `## Procedure` (always present — without it the file is not an operation). The procedure body is **always a numbered list** (`1. …`, `2. …`); single-step procedures are still numbered (`1. …`). Tool identity is named inline by the step (`cargo …`, `gitnexus_*(…)`); there is no separate `## Tools` section.
