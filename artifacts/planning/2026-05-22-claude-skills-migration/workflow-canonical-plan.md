...
references:
  ...
  meta-skill: ./sample/meta/workflow-canonical/SKILL.md
---
...
| Wire format to the agent | TOON, transcoded by the server at read time | User answer, 2026-05-22 |
...
- **Descriptive companion**: [workflow-canonical-ontology.md](./workflow-canonical-ontology.md) — human-facing; positive and negative definitions, composition rules, canonical specifier format, navigator-vs-singleton shapes, the plumber analogy, SPEM 2.0 grounding, empirical validation against the 25 legacy skills + 28 resources.
- **Operational artifact**: [sample/meta/workflow-canonical/SKILL.md](./sample/meta/workflow-canonical/SKILL.md) — agent-facing; the meta-skill the agent loads at session start to interpret skills tagged with this ontology.

The descriptive companion and the meta-skill carry the same ontology in two registers. When the two conflict, the meta-skill is authoritative for agent execution; the companion explains *why*.
...
- The deliverable-shaped competency the skill produces (Skill — named).
...
- The semantic interpretation of those skills — what makes one a "deliverable", a "technique", a "competency-bundle", a "meta-skill".
- The composition rules — how skills nest, how techniques live under owning skills, how role contracts are sectioned inside the workflow skill.
- The canonical specifier format — `skill:<path>` URIs in markdown links.
- The ontology-specific frontmatter fields — `kind`, `produces`, etc.

Crucially, **everything the ontology adds lives in the meta-skill, not the server**. Authoring the meta-skill IS authoring the ontology.
...
- **Other entry files are free-form**; the convention is `index.md` for techniques, roles, and tools.
- **Techniques live inside their owning skill** at `skills/<skill>/techniques/<technique>/index.md`. There is no top-level `techniques/` directory.
- **Skills are two-kinded**: deliverable-shaped (the existing 25) and competency-bundle (new — houses related techniques).
- **Tools, roles, workflows** are top-level peers of skills.
- **No `resources/` directory** — the 28 existing resources are fully redistributed per [Appendix C](./appendix-resource-subsumption.md).
...
workflows/
  work-package/
    ...
    skills/                                # MIGRATED.
      ...
      # --- Meta-skills (define the ontology) ---
      # Read first by the agent; bootstrap content. Other skills reference
      # the meta-skill via their `ontology:` frontmatter field.
      meta/
        workflow-canonical/
          SKILL.md                         # the workflow-canonical ontology definition
                                           #   (see sample/meta/workflow-canonical/SKILL.md)

                                           # 25 deliverable-shaped + ~7 competency-bundle = 32 total
      # --- Deliverable-shaped skills (25) ---
      # Every skill folder contains a lean SKILL.md (navigator) PLUS one or more action-named
      ...
      implement-task/
        SKILL.md                           # navigator + technique table
      ...
      # research-knowledge-base and elicit-requirements keep their navigator with one
      # named nested technique each.
      #
      # ... remaining deliverable-shaped skills follow the same navigator + named-techniques pattern ...
...
- **`SKILL.md`** at the root of every skill folder is **mandatory** (agentskills.io spec). Every level — top-level skill, competency-bundle, AND nested technique — uses this filename. On-disk shape is uniform: every folder contains exactly one `SKILL.md` and may contain nested skill folders. No flat sub-files anywhere.
...
- **Frontmatter** — server-enforced fields are the minimum needed; everything else is ontology-defined:
  ...
  - **Under `metadata:` (architecture-required)**: `ontology: workflow-canonical`, `kind: <deliverable|competency-bundle|technique|meta-skill>` — two bare-slug fields.
  - **Under `metadata:` (workflow-canonical-defined)**: `produces: <work-product-slug>` when `kind: deliverable`. Additional metadata entries per the meta-skill's schema.
...
**A deliverable-shaped skill (lean navigator) — example sketch:**
...
- **Lean.** The navigator is ~25 lines. No embedded protocol; the protocol lives in the named techniques.
...
The migration will produce per-skill navigators of this shape, grounded in the existing TOON content.
...
- The meta-skill at `meta/workflow-canonical` is the FIRST skill the agent fetches at session start when it sees a skill carrying `metadata.ontology: workflow-canonical` in frontmatter. This is an agent-side concern, not a server-side one — the server just resolves the path; the agent's bootstrap protocol decides when to fetch the meta-skill.
- The `meta/` folder convention. The workflow-canonical ontology places its meta-skill at `skills/meta/workflow-canonical/SKILL.md`. This is one ontology's chosen convention; the server doesn't require any specific location for meta-skills.
- The kind values within workflow-canonical (`deliverable`, `competency-bundle`, `technique`, `meta-skill`) — opaque to the server, interpreted by the agent via the meta-skill.
...
### Phase 0 — Author the meta-skill (`skill:meta/workflow-canonical`)

- Author `workflows/work-package/skills/meta/workflow-canonical/SKILL.md` based on the planning-folder sample at `sample/meta/workflow-canonical/SKILL.md`.
- The meta-skill defines: Skill/Technique/Role/Tool concepts, composition rules, canonical specifier format (`skill:<path>`), top-level skill shapes (navigator-style vs singleton-style), frontmatter schema, agent bootstrap procedure, refusal paths.
- Validate that the meta-skill is fetchable via `get_skill("meta/workflow-canonical")`.
- Test: an agent given only the meta-skill + an empty skills tree understands the ontology well enough to interpret a sample skill correctly.

### Phase 1 — Foundations (planning + alignment)

- ✓ Finalise the ontology (this document, pending review).
- Finalise the unified frontmatter schema for `SKILL.md` files: top-level `name` and `description` (spec-required); `metadata.ontology: workflow-canonical` and `metadata.kind: <deliverable|competency-bundle|technique|meta-skill>` (architecture-required); `metadata.produces` for deliverable skills.
...
- Sketch one example deliverable-shaped `SKILL.md`, one competency-bundle `SKILL.md`, one nested-technique `SKILL.md`, one tool entry, and one role sub-file for review before bulk migration.
- Decide on the competency-bundle skill set (proposal: `gitnexus`, `workflow`, `code-review`, `research`, `design`, `assumption-management`, `testing` — 7 bundles housing 18 techniques; may consolidate to 5–6 after authoring).

### Phase 2 — Server scaffolding (ontology-agnostic markdown loader)

- Add a minimal `SKILL.md` frontmatter schema under `src/schema/` — server-enforced fields only: `name`, `description`, `ontology`. Everything else is passed through opaquely (`kind`, `produces`, and any other meta-skill-defined fields).
...
- Extend `readSkillRaw` / `tryLoadSkill` to: (a) walk recursively into nested skill folders at arbitrary depth; (b) detect markdown sources at any path depth; (c) deliver the raw markdown verbatim on the wire (no transcoding, no ontology validation).
- Generalise `get_skill` to accept arbitrary relative paths within the workflow's `skills/` directory (e.g. `meta/workflow-canonical`, `gitnexus/impact`, `gitnexus/impact-analysis/analysis1`).
...
- Test: example markdown skill from Phase 0 (the meta-skill) and Phase 1 round-trip correctly through `get_skill` at any nesting depth.

### Phase 3 — Author tool-dedicated skills (where warranted)

- For each tool complex enough to warrant operational documentation (primarily `gitnexus`; possibly `concept-rag`), author a tool-dedicated competency-bundle skill at `skills/<tool>/SKILL.md`.
...
### Phase 5 — Author the remaining competency-bundle skills + their nested techniques

- For each remaining domain-bundle (`code-review`, `research`, `design`, `assumption-management`, `testing`): author the bundle's lean `SKILL.md` (navigator + `## Techniques` table), then author each nested technique directly under it as `<technique>/SKILL.md`.
...
### Phase 6 — Migrate the 25 deliverable-shaped skills (mechanical, one-by-one)

For each of the 25 TOON skills, in dependency order (tool-dedicated skills, workflow skill, and competency-bundles must exist first):

1. Create `workflows/work-package/skills/<skill-name>/SKILL.md` as a lean navigator (~25–80 lines). Frontmatter: top-level `name`, `description`; under `metadata:` `ontology: workflow-canonical`, `kind: deliverable`, `produces: <slug>`. Body: 1–2 paragraphs of context + a `## Techniques` table.
...
3. Build the navigator's `## Techniques` table: ordered list of every technique the protocol invokes — nested ones via `./` paths, cross-cutting ones via `../<bundle>/<technique>/SKILL.md`.
4. Move skill-level rules into the navigator (invariants/guards) OR into the relevant nested technique (in-flight discipline) OR into the role contract in `skills/workflow/SKILL.md` (cross-cutting role behaviour like the gitnexus discipline).
...
- `dco-provenance` (TOON 25): **collapse**. Do not author a navigator. Update `workflow.toon` activities that previously bound to `dco-provenance` to bind directly to `workflow/dco-attest-commit/SKILL.md` instead.
- `elicit-requirements` (TOON 05) and `research-knowledge-base` (TOON 06): **keep with one named technique each**. The navigator preserves skill-specific framing (elicitation style, KB-research synthesis logic) that goes beyond the underlying technique.
...
- Update `docs/api-reference.md` for the generalised `get_skill` and the new `get_tool` / `get_subfile` tools.
...
2. **The markdown parser and the MCP tools** (`get_technique`, `get_role`) are workflow-agnostic.
...
- After migration, no rule or protocol element that was previously inside a skill body has been silently relocated to `workflow.toon` or `activities/*.toon`. (Relocation to techniques is fine — workers fetch them via `get_technique`.)
...
| Competency-bundle skills accumulate too many techniques and lose coherence | Medium | Medium | Per-bundle technique limit (soft cap: ~6 techniques per bundle); split when exceeded. |
...
1. **Cross-workflow technique sharing.** When 8 of 10 workflows need `gitnexus` techniques, should the `gitnexus` competency-bundle skill live at the top-level shared layer (`workflows/_shared/skills/gitnexus/`) or be duplicated per workflow? Decision deferred to post-pilot.
...
4. **The "Activity" concept's home.** Activities currently live in `activities/*.toon`. With the new ontology, activities are the workflow-structural binding of role-name + deliverable-shaped skill. Do they stay separate, or fold into `workflow.toon`? Out of scope for the pilot — but worth a future RFC.
5. **Competency-bundle skill set finalisation.** The proposed 7 bundles (`gitnexus`, `workflow`, `code-review`, `research`, `design`, `assumption-management`, `testing`) is the starting point. Empirical validation during Phases 3–5 may consolidate to 5–6 bundles; this is expected.
6. **Tool-dedicated skills beyond gitnexus.** The pilot creates a tool-dedicated skill for `gitnexus`. Other tools (`concept-rag`, `git`, `cargo`) MAY warrant the same treatment if usage volume justifies it. Default for the pilot: only `gitnexus` gets a dedicated skill; others appear inline in technique prose. Re-evaluate post-pilot.
...
8. **Meta-skill versioning.** Should the meta-skill itself be versioned (`skill:meta/workflow-canonical-v1`, `-v2`, etc.)? Skills frozen at v1 stay interpretable even if v2 introduces breaking changes. Open until the meta-skill needs its first revision.
...
- Cross-workflow shared library design for techniques OR competency-bundle skills (deferred — see open question 1).
...
1. **The ontology** — see [workflow-canonical-ontology.md](./workflow-canonical-ontology.md) and the meta-skill at [sample/meta/workflow-canonical/SKILL.md](./sample/meta/workflow-canonical/SKILL.md).
...
- [workflow-canonical-ontology.md](./workflow-canonical-ontology.md) — the descriptive companion to the workflow-canonical meta-skill.
- [sample/meta/workflow-canonical/SKILL.md](./sample/meta/workflow-canonical/SKILL.md) — the agent-facing meta-skill (operational form).
