...
The workflow server records claims and magnitudes but never reconciles or aggregates them: worker-declared artifacts are never diffed against the planning folder before that folder is committed, per-activity token rows are never rolled up, block-level delivery dedup is unmeasured against the ship gate, and three specified observability pieces (resource validation, step-granularity events, per-agent filtering) were never delivered. The cost is concrete — undeclared files already reached an unreviewed commit, repeated material is paid for on every delivery, and one worker's technique fetch currently counts as fidelity coverage for a sibling worker's step. Scope is fixed to items S2, S3 tail, S4 and S5 of #365; system context, impact and classification are in [design philosophy](02-design-philosophy.md).

## Goal

Every discrepancy the server can detect is surfaced to the agent that can act on it, and every magnitude it records is summable and attributable to the agent that incurred it — without changing the behaviour of any existing run. (Money pricing of the usage aggregate is out of this package — see [deferred-items](deferred-items.md) D-4.)
...
| Workflow operator / issue author | A per-run token total they can compare between approaches | As an operator, I want a per-workflow usage aggregate so that I can see run token spend and notice when it goes wrong (derived money pricing deferred — D-4) |
| Agent or human debugging a multi-worker run | To attribute events, fetches and usage to the worker that caused them | As a debugger, I want trace, history and usage filterable by agent so that a parallel run's evidence is separable per worker |
...
3. **S3 aggregate (tokens only).** A per-workflow usage aggregate that is the plain sum of DELTA rows (optional `agent_id` attribution). Price capture and any derived money figure are out of this package ([deferred-items](deferred-items.md) D-4; RE-4 corrected).
...
| SC-5 | `inspect_session view: usage` exposes a per-workflow **token** aggregate equal to the plain sum of DELTA rows (with optional per-agent filter per SC-12); no derived money/cost field is required in this package | Same fixture as SC-4 plus assertion that the view returns the summed token fields and does not require a price table; cost/price capture covered by D-4 |
...
**Confirmed by:** User
**Date:** 2026-07-31
**Notes:** Six decisions settled one at a time in direct stakeholder interview, each with a recommendation and its rejected alternatives. Research residue RE-4 resolved 2026-07-31: stakeholder "defer price capture. token usage is sufficient" — SC-5 reframed to token aggregate; cost/price capture in [deferred-items](deferred-items.md) D-4.
