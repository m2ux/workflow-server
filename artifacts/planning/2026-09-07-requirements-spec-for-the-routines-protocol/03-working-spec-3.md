...
**REQ-F040: A routine SHALL run inside the referring activity's existing dispatch.**

*Status*: *pending*

*Rationale*: The execution locus is the reason a routine is its own kind of definition rather than a way of reaching an existing activity; the dispatch cost this locus holds unchanged is REQ-NF011's obligation. Promoting the shared run to an activity that four activities route through would cost four extra hand-offs whose only content is a gate and two record passes, and a guard already forbids an activity opening with a decision — which this run does.
