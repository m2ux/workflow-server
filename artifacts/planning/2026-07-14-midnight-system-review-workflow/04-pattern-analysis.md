...
| S5 | Toolchain gate idiom: probe step runs the tool; a `set` action after it flips the gate true (reaching the action implies success; `defaultValue: false` covers failure); rules route capability vs fallback on the gate | `substrate 01-scope-setup.yaml:16-25` (`gitnexus-operations::analyze` → `set gitnexus_available`); `substrate workflow.yaml:19` (graph-vs-grep routing rule) | `detect-toolchain` emits `gitnexus_available` / `cargo_available` / `node_binary_available` as declared boolean outputs (landing via the sanctioned `variables_changed` path); graceful-degradation rule routes probes | Adapt (quality-review CF-1): one op probes three toolchains with per-toolchain conditional outcomes, which the unconditional post-probe `set` idiom cannot express; `defaultValue: false` still covers failure |
