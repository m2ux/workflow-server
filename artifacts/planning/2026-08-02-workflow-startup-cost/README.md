...
4. **Client first activity fetched** — the first `next_activity` call naming the client workflow's opening activity, `start-work-package` on eight of the nine runs; everything before this is the meta workflow's ceremony (discover-session, initialize-session, resolve-target, dispatch-client-workflow).
