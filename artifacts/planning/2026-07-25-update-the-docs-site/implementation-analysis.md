...
|----|-----|-------------------|-----------|----------|-------------------|----------------|
| C-01 | H | “The server registers **17** MCP tools” is correct in README/api page lede, but other pages say **sixteen** | `site/index.html` (diagram desc), `site/design/request-lifecycle.html`, `site/design/server-anatomy.html` (“sixteen closures”) | Live registration: 13 + 4 = **17** named tools (`discover`…`get_resource`) | Always say **17 MCP tools**; regenerate site prose if hand-authored | Site index, design pages; keep README/api lede aligned after regen |
| C-02 | M | `workflow-tools.ts` registers “**twelve** navigation, checkpoint, and trace tools” | `site/design/server-anatomy.html` | `server.tool(` count in `workflow-tools.ts` = **13** (includes bootstrap trio + navigation + checkpoints + trace + status/inspect) | “**13** tools in `workflow-tools.ts`; **4** in `resource-tools.ts`” | server-anatomy |
