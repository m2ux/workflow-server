...
| 2 | `techniques/publish-review-artifacts.md` | technique | modify | Orphan `reference_path` input becomes `repo_root`; both git reads re-target the engineering checkout via a `{$eng_git_dir}` local; output description de-proceduralised. **Delegated op (post-update remediation of [F-1](10-post-update-review.md)):** step 2 moves from `version-control::commit-regular-files`, which is parent-repo-scoped and declares no `repo_path`, to `manage-git::artifact-commits`, which stages, commits and pushes inside the engineering checkout — so the git *write* addresses the same checkout as the reads. The canonical message pattern replaces the literal one, supplying `activity_name` and `issue_key`. |
...
**AP-129 sweep — both halves, completing [F-3](10-post-update-review.md).** The first pass swept only the retired *symbol* (`reference_path`, 0 occurrences tree-wide) and not the altered *behavioural claim*, which is what AP-129 specifies; it also left the delegated op unenumerated, which is how the parent-repo write reached commit. Both are now closed. The claim sweep — restatements that planning artifacts are committed or pushed to the parent repo — returns **no hit inside the four-file surface**. Two hits sit outside it: `techniques/manage-artifacts/TECHNIQUE.md:72` (the `committed-to-parent` rule) and `meta/techniques/version-control/TECHNIQUE.md:8` (a group capability line that accurately describes that op family's scope). The first is one of four inherited sites carrying this class, each verified byte-identical to `origin/workflows` and tracked in [#319](https://github.com/m2ux/workflow-server/issues/319) rather than fixed here, since `agent-conduct.md` among them sits in `meta`. **Enumerated delegated op:** step 2 of `publish-review-artifacts.md` delegates the git write, so the op it names is part of the surface whenever that technique's git target changes.
