#!/usr/bin/env bash
# Deploy a Cursor multi-root workspace from this checkout.
#
# This file lives on the workspace branch. The branch root is the template.
# For workflow-server, machine-local files are rendered into this checkout.
# The local-share workspaces directory is not used for that repo.
# Any other repo name is still rendered under
# $HOME/.local/share/cursor/workspaces/<name>/ with absolute folder paths.
#
# workflow-server layout, relative to this checkout:
#   🏠 workspace   → ./
#   📂 project     → ./components/main
#   🔀 workflows   → ./components/workflows
#   📋 planning    → ./engineering/artifacts/planning
...
# This checkout is the template. --template replaces it.
...
Render a Cursor workspace from this checkout.

workflow-server writes machine-local files into this checkout. The relative
tool links are already committed here and are left in place. Any other repo
name is copied to Cursor's workspaces dir with absolute paths under \$HOME/….
...
Options:
  ...
  --template=DIR             Template source (default: this checkout, the parent of scripts/)
...
if [[ "$REPO_BASENAME" == "workflow-server" ]]; then
  IN_PLACE=1
  DEST_DIR="$TEMPLATE_DIR"
  PROJECT_DIR="${TEMPLATE_DIR}/components/main"
  WORKTREES_DIR="${TEMPLATE_DIR}/.worktrees"
  WORKFLOWS_DIR="${TEMPLATE_DIR}/components/workflows"
  PLANNING_DIR="${TEMPLATE_DIR}/engineering/artifacts/planning"
...
if [[ "$IN_PLACE" -eq 0 && -e "$DEST_DIR" && "$FORCE" -ne 1 && "$DRY_RUN" -ne 1 ]]; then
  die "destination exists: ${DEST_DIR} (re-run with --force to overwrite managed files)"
fi
if [[ "$IN_PLACE" -eq 0 && -e "$DEST_DIR" && "$DRY_RUN" -eq 1 && "$FORCE" -ne 1 ]]; then
...
if [[ "$IN_PLACE" -eq 1 ]]; then
  log "in-place: keep committed scripts, config, and .claude/hooks"
elif [[ -d "${TEMPLATE_DIR}/scripts" ]]; then
  if [[ "$DRY_RUN" -eq 1 ]]; then
...
if [[ "$IN_PLACE" -eq 1 ]]; then
  log "in-place: keep committed .mcp.json"
else
...
# --- .code-workspace (absolute $HOME paths for a copied kickoff) --------------
# shellcheck disable=SC2016
if [[ "$IN_PLACE" -eq 1 ]]; then
  log "in-place: keep committed code-workspace"
else
WORKSPACE_JSON=$(
  PROJECT_DIR="$PROJECT_DIR" \
...
if [[ "$IN_PLACE" -eq 1 ]]; then
  log "in-place: keep committed AGENTS.md and tool links"
elif [[ "$DRY_RUN" -eq 1 ]]; then
...
if [[ "$SKIP_MKDIR" -eq 0 && "$IN_PLACE" -eq 0 ]]; then
  if [[ ! -d "$PROJECT_DIR" ]]; then
...
echo
if [[ "$IN_PLACE" -eq 1 && ! -d "$PROJECT_DIR" ]]; then
  echo "Next: add worktrees of main, workflows, and engineering under this checkout."
  echo
