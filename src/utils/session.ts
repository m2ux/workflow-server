 ...
 *
 * Replaces the legacy `assertCheckpointsResolved(SessionPayload)` which read
 * `token.bcp`; now reads `state.activeCheckpoint` per PD-11.
 */
...
/**
 * Legacy alias retained for any internal call sites that still operate on a
 * decoded `SessionPayload` (start_session's transitional path). Authenticated
 * tools should call `assertNoActiveCheckpoint(state)` against the loaded
 * `SessionFile` instead.
 */
