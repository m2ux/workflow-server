  ...
  server.registerTool(
    'start_session',
    {
      description:
        ...
        'STRICT PARAMETERS (applies when running without the harness interceptor; see docs/interceptor-recipe.md): this tool rejects unknown keys (e.g., do NOT pass "saved_session_token" — pass the saved token under the "session_token" parameter). With the interceptor installed the LLM does not construct parameter names and this constraint is satisfied automatically. ' +
        'STALENESS RECOVERY POLICY (applies when running without the harness interceptor): HMAC staleness recovery (re-signing a saved token after a server restart) is performed ONLY by start_session. Other workflow tools (next_activity, get_workflow, etc.) verify HMAC strictly with no recovery. To recover a stale saved token, call start_session with session_token set to the saved value; the auto-adopt path re-signs the payload in place and preserves sid, act, and variables. The interceptor is the recommended fix for the transcription-drift failure mode this policy compensates for; staleness recovery remains useful for genuine server-restart cases.',
      inputSchema: z
        .object({
