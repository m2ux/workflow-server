...
export const WorkflowSchema = z.object({
  ...
  initialActivity: z.string().describe('ID of the first activity to execute. Every workflow declares one: it is the id the first `next_activity` names, and the root the reachability half of the activity-variables guard walks from. A workflow without it loads, and then reports every variable as available at every activity because the walk has no entry point to start from.'),
