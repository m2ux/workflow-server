...
- Identifier case convention (AP-55): scan every technique's input/output/protocol-variable ids for snake_case (or any non-kebab spelling). Flag each for conversion to kebab-case, UNLESS it mirrors an external tool/MCP/CLI parameter (cloudId, owner, repo, issueIdOrKey — keep the tool's spelling). Confirm direction is taken from the `## Inputs`/`## Output` section, not the case; a value that is an output of one technique and an input of others should be one kebab symbol declared in both sections, not two differently-cased ids
