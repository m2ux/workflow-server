...
- Designator bracing (AP-49): for every technique, scan `## Protocol` for references to a declared input/output (or a value of one) written without the brace designator — a bare identifier (`target-path`, `problem_statement`) or an orphan value (a bare index/enum value like "index 23"). Flag each; recommend bracing as `{declared-id}` (normalized to the declared spelling) or, for orphan values, naming the input ("when {lens-resource-index} is 23"). Do NOT flag ordinary English words that merely coincide with an id
