...
- Reference backticking (AP-59): every designator reference in rendered prose — `{id}` / `{id}.field`, protocol-variable `{$name}` / `{name}`, and dotted rule address `technique.rule-name` — is wrapped in backticks. Flag any bare-in-prose designator (a `{…}` or dotted rule ref that is NOT inside a backtick code span and NOT already backticked). Do NOT flag a reference already inside a larger code span (a command, a path expression). Also flag any surviving `{\$name}` escape — it is obsolete now that the backticked sigil is math-exempt; de-escape it to `` `{$name}` ``
