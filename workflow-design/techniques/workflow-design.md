...
- Identifier case convention (AP-55): scan every technique's input/output/protocol-variable ids for snake_case or camelCase (any non-kebab spelling) and flag each for conversion to kebab-case — with NO exception, including ids that map to an external tool/MCP/CLI parameter (cloudId → cloud-id, issueIdOrKey → issue-id-or-key, session_index → session-index). Tool interop is NOT handled by per-id binding notes — it is handled by the agent-side separator-equivalence resolution rule (`-` ≡ `_`); flag and remove any leftover "Passed as `X` to the … tools" binding notes. Confirm direction is taken from the `## Inputs`/`## Output` section, not the case; a value that is an output of one technique and an input of others should be one kebab symbol declared in both sections, not two differently-cased ids
