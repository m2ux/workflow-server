...
- Walk every anti-pattern in [anti-patterns](../resources/anti-patterns.md) (currently 60 entries across 8 categories); for each finding, record file, content, and recommended fix
...
- Designator bracing (AP-49): for every technique, scan `## Protocol` for references to a declared input/output (or a value of one) written without the brace designator — a bare identifier (`target-path`, `problem_statement`) or an orphan value (a bare index/enum value like "index 23"). Flag each; recommend bracing as `{declared_id}` (normalized to the declared spelling) or, for orphan values, naming the input ("when {lens_resource_index} is 23"). Do NOT flag ordinary English words that merely coincide with an id
...
- If prose is used where a formal schema construct exists, check the [schema construct inventory](../resources/schema-construct-inventory.md) and replace the prose with the formal construct
...
- Commit the completed `workflow-files` to the workflows worktree with a signed commit
