...
*Prefer / before / only after* stance — broader than any one defect; avoids families of smells. No Detect triad here; specific bad instances live in the anti-pattern catalog. Includes **§1 Workflows Ossify Patterns** (why workflows exist — reuse of fixed patterns vs on-the-fly novelty), **§2 Activities Coordinate; Techniques Endow** (layer map + placement test: activity coordinates fan-out/dispatch; technique endows atomic capability; resource consults; workflow graph is session scope), **§27 Atomic Techniques; Compose at Activities**, **§29 Creation Guide for Generated Documents** (every persisted planning artifact maps to a Template+Rules guide), **§30 Cite Resource Policy; Do Not Restate It** (technique Protocol cites resource policy; does not re-author the matrix), **§31 Resources at the Abstract Level; Split for Section Delivery** (resources describe abstract kinds/roles and template skeletons — concrete artifact names and variable bindings live in the technique — multi-part resources split into per-category sections; content a section-scoped reader depends on lives in a section), **§33 Cite Resources at Section Grain** (citations name the narrowest deliverable section), and **§34 Prefer Parallel Independent Work via Formal Fan-Out** (prefer parallelising independent work via **pattern activities** under `meta/activities/patterns/` — agent patterns 01–05; process-unit suite as pattern or mirrored spine — serial only when dependence, shared mutation, or host limits require it; fan-out is not a technique home).

### 01 — Schema Construct Inventory

Layer map plus four mapping tables that translate informal prose patterns into formal schema equivalents (activity, workflow, technique, condition). Fan-out and worker dispatch map to **borrowable pattern activities**, not strategy techniques.

### 02 — Anti-Patterns

Specific smell instances — Detect / Do not flag / Fix. Cite by kebab-case smell **name** only (not AP numbers). Section-delivery family includes `whole-resource-for-one-section` and `framing-outside-any-section`. Technique locus family includes `technique-stage-agnostic` (activity-blind technique prose) and `container-names-inheriting-ops` (group/root contracts do not name descendant ops that inherit them). Dispatch / coordination family includes `prose-based-dispatch-patterns`, **`coordination-in-technique`** (multi-unit scatter/wait-all/gather prefers activity coordination), and `pass-orchestration-in-technique` (multi-op Protocol façades). Peer technique cites for documentation or single-capability work remain allowed (`canonical-technique-reference`).
