...
Specific smell instances — Detect / Do not flag / Fix. Cite by kebab-case smell **name**. Section-delivery family includes `whole-resource-for-one-section` and `framing-outside-any-section`. Dispatch family includes `prose-based-dispatch-patterns` (free concurrent fan-out recipes that re-teach shells, agents, Tasks, wait-all, or ordered gather without Applying the unit-kind contract).
