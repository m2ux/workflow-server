...
"Fan out four concurrent shells… Wait for ALL four…" / "dispatch them concurrently (up to four at once)" as free Protocol or Rules without a named fan-out contract bound at the activity

A technique re-teaches concurrent fan-out — shells, agents, Tasks, wait-all, ordered gather — in free Protocol or Rules prose instead of leaving that verb to the formal home bound in activity structure for the unit kind.

**Detect:** Technique `## Protocol` or `## Rules` embeds a concurrency recipe (concurrent shells/processes, concurrent agent or Task dispatch, wait-all, ordered gather, combine-after-fan-out) whose verb is already owned by a formal home — [`unit-fan-out`](../../meta/techniques/unit-fan-out.md) for same-context process/shell/tool units; [`scatter-gather`](../../meta/techniques/scatter-gather.md) parallel mode, [`spawn-concurrent`](../../meta/techniques/harness-compat/spawn-concurrent.md), or [`orchestration-patterns`](../../meta/techniques/orchestration-patterns/TECHNIQUE.md) for agent/lens units — and the technique neither is that formal home nor is limited to non-dispatch work (e.g. pure combine over already-gathered results). Test: strip the free concurrency HOW and the remaining novelty is only the unit roster, domain budgets, or product-specific combine; the dispatch/gather verb is already named elsewhere.

**Do not flag:** The formal homes themselves (their Protocols *are* the contract). Plan or inventory metadata that records parallelism intent without teaching runtime dispatch. Coordination locks, flocks, and rebase sibling races (mutual exclusion, not fan-out). Graph or complexity metrics that use "fan-out" as a structural measure. Atomic techniques that only fold or analyse already-bound inputs without teaching fan-out. Activity YAML that binds the unit-kind-correct contract as a step (composition belongs there).

**Fix:** In **activity** structure, bind the unit-kind-correct contract as a step (`unit-fan-out` for process/shell/tool units; `scatter-gather` parallel and/or `spawn-concurrent` / orchestration-patterns for agent units); delete the free concurrent recipe from the technique Protocol/Rules; keep domain envelope as activity step inputs or adjacent steps. Technique prose states only this op's inputs → work → outputs — it does not name activities or who binds sibling steps (`technique-stage-agnostic`). Do **not** Protocol-Apply the fan-out contract from a façade technique — that is `pass-orchestration-in-technique` (AP-114). See [Prefer Parallel Independent Work via Formal Fan-Out](./design-principles.md#33-prefer-parallel-independent-work-via-formal-fan-out), [Keep Orchestration in Structure](./design-principles.md#20-keep-orchestration-in-structure), [Atomic Techniques; Compose at Activities](./design-principles.md#26-atomic-techniques-compose-at-activities), [Prefer Shared Capability](./design-principles.md#18-prefer-shared-capability). Related: `duplicate-shared-capability`, `pass-orchestration-in-technique`, `technique-stage-agnostic`.
