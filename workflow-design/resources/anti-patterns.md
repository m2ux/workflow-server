...
"Fan out four concurrent shells… Wait for ALL four…" / "dispatch them concurrently (up to four at once)" as free Protocol or Rules without Apply of a named fan-out contract

A technique re-teaches concurrent fan-out — shells, agents, Tasks, wait-all, ordered gather — in free Protocol or Rules prose instead of Applying the formal contract that owns that unit kind.

**Detect:** Technique `## Protocol` or `## Rules` embeds a concurrency recipe (concurrent shells/processes, concurrent agent or Task dispatch, wait-all, ordered gather, combine-after-fan-out) whose verb is already owned by a formal home — [`unit-fan-out`](../../meta/techniques/unit-fan-out.md) for same-context process/shell/tool units; [`scatter-gather`](../../meta/techniques/scatter-gather.md) parallel mode, [`spawn-concurrent`](../../meta/techniques/harness-compat/spawn-concurrent.md), or [`orchestration-patterns`](../../meta/techniques/orchestration-patterns/TECHNIQUE.md) for agent/lens units — and the site does not Apply or Honor that contract. Test: strip the free concurrency HOW and the remaining novelty is only the unit roster, domain budgets, or product-specific combine; the dispatch/gather verb is already named elsewhere.

**Do not flag:** The formal homes themselves (their Protocols *are* the contract). Plan or inventory metadata that records parallelism intent without teaching runtime dispatch. Coordination locks, flocks, and rebase sibling races (mutual exclusion, not fan-out). Graph or complexity metrics that use "fan-out" as a structural measure. Call sites that already Apply or Honor the unit-kind-correct contract and retain only domain envelope (budgets, backoff, product composition).

**Fix:** Apply the unit-kind-correct contract (`unit-fan-out` for process/shell/tool units; `scatter-gather` parallel and/or `spawn-concurrent` / orchestration-patterns for agent units); delete the free concurrent recipe from Protocol/Rules; keep caller-owned domain envelope beside the Apply. See [Prefer Parallel Independent Work via Formal Fan-Out](./design-principles.md#33-prefer-parallel-independent-work-via-formal-fan-out), [Prefer Shared Capability](./design-principles.md#18-prefer-shared-capability), and [Maximize Schema Expressiveness](./design-principles.md#5-maximize-schema-expressiveness). Related: `duplicate-shared-capability`.
