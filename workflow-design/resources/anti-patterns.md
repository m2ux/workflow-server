...
A technique re-teaches concurrent fan-out — shells, agents, Tasks, wait-all, ordered gather — in free Protocol or Rules prose instead of relying on the formal contract the **binding activity** binds as a step for that unit kind.

**Detect:** Technique `## Protocol` or `## Rules` embeds a concurrency recipe (concurrent shells/processes, concurrent agent or Task dispatch, wait-all, ordered gather, combine-after-fan-out) whose verb is already owned by a formal home — [`unit-fan-out`](../../meta/techniques/unit-fan-out.md) for same-context process/shell/tool units; [`scatter-gather`](../../meta/techniques/scatter-gather.md) parallel mode, [`spawn-concurrent`](../../meta/techniques/harness-compat/spawn-concurrent.md), or [`orchestration-patterns`](../../meta/techniques/orchestration-patterns/TECHNIQUE.md) for agent/lens units — and neither the technique nor its binding activity binds that contract as an activity step. Test: strip the free concurrency HOW and the remaining novelty is only the unit roster, domain budgets, or product-specific combine; the dispatch/gather verb is already named elsewhere.

**Do not flag:** The formal homes themselves (their Protocols *are* the contract). Plan or inventory metadata that records parallelism intent without teaching runtime dispatch. Coordination locks, flocks, and rebase sibling races (mutual exclusion, not fan-out). Graph or complexity metrics that use "fan-out" as a structural measure. Binding activities that already bind the unit-kind-correct contract as a step and retain only domain envelope (budgets, backoff, product composition) on adjacent steps. Atomic techniques that document activity-owned parallel dispatch without Protocol Apply of the fan-out contract.

**Fix:** Bind the unit-kind-correct contract as an **activity step** (`unit-fan-out` for process/shell/tool units; `scatter-gather` parallel and/or `spawn-concurrent` / orchestration-patterns for agent units); delete the free concurrent recipe from the technique Protocol/Rules; keep domain envelope on the activity (unit roster, budgets, backoff) as step inputs or adjacent steps. Do **not** Protocol-Apply the fan-out contract from a façade technique — that is `pass-orchestration-in-technique` (AP-114). See [Prefer Parallel Independent Work via Formal Fan-Out](./design-principles.md#33-prefer-parallel-independent-work-via-formal-fan-out), [Atomic Techniques; Compose at Activities](./design-principles.md#26-atomic-techniques-compose-at-activities), [Prefer Shared Capability](./design-principles.md#18-prefer-shared-capability), and [Maximize Schema Expressiveness](./design-principles.md#5-maximize-schema-expressiveness). Related: `duplicate-shared-capability`, `pass-orchestration-in-technique`.
