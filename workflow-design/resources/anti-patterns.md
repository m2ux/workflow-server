...
**Do not flag:** Purpose/value orientation without a pass inventory; pointers to the YAML; at-a-glance activity names with one-line roles (`readme-orients-not-transcribes`). Technique bodies that name or Apply sibling techniques are `technique-references-technique` / `pass-orchestration-in-technique` — not a Do-not-flag here.
...
**Fix:** Strip every technique reference (Apply, `::`, hyperlink, prose name); bind each needed op as its own activity step in the order required; keep only distinct local value assembly (if any) as a separate atomic technique over tools and resources. See AP-142, [Bind Sibling Operations as Steps](./design-principles.md#26-bind-sibling-operations-as-steps), [Atomic Techniques; Compose at Activities](./design-principles.md#27-atomic-techniques-compose-at-activities). Related: `prose-based-dispatch-patterns` (AP-140), `coordination-in-technique` (AP-143).
...
**Fix:** Rewrite the I/O description as bind-contract meaning/shape only. Composition lives in activity `steps[]`. See AP-142, [Separate Contract from Procedure](./design-principles.md#14-separate-contract-from-procedure); also `io-agnostic-contract`.
...
**Fix:** Delete every technique→technique reference. If the missing edge is **coordination** (who runs when, concurrency, wait-all, gather of peers), lift it to an **activity** or pattern activity under `meta/activities/patterns/`. If the missing edge is real work sequencing of atomic capabilities, bind each op as its own activity step in order. If the missing edge is shared vocabulary or policy, put it in a **resource** and cite the resource. If the missing edge is only scope colour ("not me — X does"), keep a bare negation or positive capability with no peer name. See [Activities Coordinate; Techniques Endow](./design-principles.md#2-activities-coordinate-techniques-endow), [Atomic Techniques; Compose at Activities](./design-principles.md#27-atomic-techniques-compose-at-activities), [Bind Sibling Operations as Steps](./design-principles.md#26-bind-sibling-operations-as-steps). Subsumes and strengthens: `pass-orchestration-in-technique` (Apply is one form), `technique-ref-in-io-contract`, `foreign-owner` / scope-negation peer names, `container-names-inheriting-ops` when the named child is cited as a peer technique from the container body. Related: `coordination-in-technique`, `prose-based-dispatch-patterns`.
...
A technique body owns **multi-unit coordination** — N independent units, concurrency bound, wait-all, ordered gather of peers — rather than a single capability product. Fan-out and worker dispatch are activity constructs.

**Detect:** Technique Capability, Protocol, or Rules defines scatter/dispatch of multiple independent units, a concurrency bound across those units, wait-all, and/or ordered gather of peer results as *this* op's job. Signals: strategy technique whose sole purpose is fan-out; Protocol phases that are only "dispatch all → wait → collect"; Outputs that are solely the gathered bag of peer runs with no domain product of one capability. Test: apply the placement test in [Activities Coordinate; Techniques Endow](./design-principles.md#2-activities-coordinate-techniques-endow) — if deleting the prose breaks who-runs-when / how-many / when-merge, the home is an activity. Distinct from a pure-combine technique that only folds already-gathered `{unit_results}`.

**Do not flag:** Atomic single-unit invoke (one process, one tool call, one worker body). Pure combine/fold/synthesise over inputs already produced by prior activity steps. Pattern activities and activity `steps[]` / loops that coordinate fan-out. Capability leaves bound *by* those activities. Free concurrent prose without claiming a coordination Capability is `prose-based-dispatch-patterns`. Technique→technique Apply of dispatch peers is `pass-orchestration-in-technique` / `technique-references-technique`.

**Fix:** Delete or refuse the coordination technique. Author or borrow a pattern activity (or local activity spine) under `meta/activities/patterns/` that owns roster, concurrency, wait-all, and gather; bind atomic unit-body and pure-combine techniques as steps. See [Activities Coordinate; Techniques Endow](./design-principles.md#2-activities-coordinate-techniques-endow), [Prefer Parallel Independent Work via Formal Fan-Out](./design-principles.md#34-prefer-parallel-independent-work-via-formal-fan-out), [Atomic Techniques; Compose at Activities](./design-principles.md#27-atomic-techniques-compose-at-activities). Related: `prose-based-dispatch-patterns`, `pass-orchestration-in-technique`, `technique-references-technique`, `technique-stage-agnostic`.
