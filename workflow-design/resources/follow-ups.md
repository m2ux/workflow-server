---
name: follow-ups
description: Template and rules for the in-task follow-ups register (work still inside the current design session).
metadata:
  version: 1.0.0
---
...
> [Design session] · `{workflow-id}` · updated YYYY-MM-DD
...
- **In-task only** — rows are work that must finish (or explicitly drop) before session close-out. Conscious out-of-scope deferrals go to the deferred-items register.
