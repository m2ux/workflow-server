...
Prose that applies on one path only is set apart from prose that always applies. Within a Protocol step, the unconditional instruction is the bullet, and the *when* / *if* / *otherwise* branches are bulleted items inside a `>` note beneath it — one item per branch, written `  > - When …`. The bullet keeps alternatives reading as the list they are; the leading `>` keeps the whole block out of the step sequence, because the protocol parser's step regex matches a bullet at any indent and a line opening with `>` fails that match and folds into the instruction above. A reader scanning the step then sees the shape of the work without evaluating every clause to learn which parts apply to their run, and the branch stays visually attached to the instruction it qualifies rather than reading as another thing to do.
