...
| **Technique** | Atomic capability: inputs → tools and resources → outputs. Endows what a bound step or dispatched worker *can produce*. No technique call graph. |
...
Fan-out and worker dispatch — single or parallel — are activity coordination. Prefer borrowing or mirroring a pattern activity before inventing a strategy technique whose Capability is scatter / wait-all / gather. Avoids `coordination-in-technique`, `technique-references-technique`, `pass-orchestration-in-technique`, `prose-based-dispatch-patterns`, and `technique-stage-agnostic`.
...
Definition prose (`description`, `outcome`, option text, README orientation for the defined workflow) states what the system *is* or *does* in positive declarative present tense — not avoidance or comparative framing against a prior design. Where a technique must bound its scope, a bare negation of excluded verbs is enough — never name another technique as the owner of the excluded work (`technique-references-technique`).
...
All multi-technique work lives in activity `steps[]` (and checkpoints/loops). Bind each already-defined sibling or shared operation as its own activity step. A technique owns one capability's produce path over **tools and resources** only — its body never names, links, `::`-cites, or `Apply`s another technique (`technique-references-technique`). Loader ancestor wrap (`Initial`/`Final`) and container I/O merge are platform composition, not a prose call graph.

## 27. Atomic Techniques; Compose at Activities

Techniques are small, relatively atomic capabilities: a short produce path over tools and resources, without complex conditional/branching orchestration and **without any reference to other techniques**. If the logic seems to require another technique — as work invoke, documentation cite, scope counterpart, or I/O producer/consumer — the wrong abstraction is in use: lift that edge to the **activity** (bind both ops as steps) or move shared knowledge into a **resource**. If the logic is multi-unit dispatch, wait-all, or gather of peers, it is **activity coordination** — borrow or mirror a pattern under [`meta/activities/patterns/`](../../meta/activities/patterns/README.md); do not mint a fan-out technique. Activities are the composition layer — they bind techniques (and checkpoints/loops) into useful work. Activity→activity composition is allowed: borrow, bind, or include activities to reuse standalone orchestration patterns. See [Activities Coordinate; Techniques Endow](#2-activities-coordinate-techniques-endow). Avoids `technique-references-technique`, `pass-orchestration-in-technique`, `coordination-in-technique`.
...
Serial execution remains correct when units depend on each other, share mutable state that concurrency would race, or the host cannot absorb concurrent load — not as the default when independence is already clear. Domain envelopes (resource budgets, backoff, product-specific composition) are step inputs and adjacent steps on the activity that coordinates the suite; the **activity** owns ordered scatter, wait-all, and ordered gather. Techniques never reference other techniques ([Atomic Techniques; Compose at Activities](#27-atomic-techniques-compose-at-activities); `technique-references-technique`). Technique definition prose stays activity-blind ([Keep Orchestration in Structure](#21-keep-orchestration-in-structure); `technique-stage-agnostic`). Fan-out is not a technique home ([Activities Coordinate; Techniques Endow](#2-activities-coordinate-techniques-endow); `coordination-in-technique`).

Avoids `prose-based-dispatch-patterns`, `coordination-in-technique`, `technique-references-technique`, `pass-orchestration-in-technique`, and `technique-stage-agnostic`. Complements [Prefer Shared Capability](#19-prefer-shared-capability) and [Maximize Schema Expressiveness](#6-maximize-schema-expressiveness).
