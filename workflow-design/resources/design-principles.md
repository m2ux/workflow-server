...
Grain is a property of the resource across the whole technique, not of one link. Every distinct citation is delivered, so a bare citation standing beside anchored ones sends the file *and* those sections, and the file's size counts against the eager budget that decides what else arrives. Where a technique needs several sections, cite each by anchor; where the set approaches the whole body, cite the resource once and drop the anchors. This is the citation half of the section-or-whole economy, whose structural half is [Resources at the Abstract Level; Split for Section Delivery](#30-resources-at-the-abstract-level-split-for-section-delivery) — a resource is split so its sections are deliverable, and cited at the grain it was split for. Avoids `whole-resource-for-one-section`.
