...
| "subagent-isolation / each unit its own commit" | **Graph, an instance fan declaring `isolation: worktree`** | A worker shares its caller's workspace and a plain fan's branches share one working tree, so neither gives a unit a checkout of its own and the load refuses a fanned activity binding a version-control operation. Declaring `isolation: worktree` on the destination lifts that refusal for its activity. It commits the author to the shape: the activity before the fan materialises one worktree per element and records its path and branch there, each instance commits its own checkout, and the activity the fan converges on reconciles the branches. Session persistence stays at convergence — the record and planning folder are shared however the checkouts are split. Costs a full checkout per branch and makes the convergence responsible for a merge that can conflict, so reach for it only where per-unit attribution is the point. |
