...
| "subagent-isolation / each unit its own commit" | **Graph, an instance fan declaring `isolation: worktree`** | A worker shares its caller's workspace and a plain fan's branches share one working tree, so neither gives a unit a checkout of its own and the load refuses a fanned activity binding a version-control operation. Declaring `isolation: worktree` on the destination lifts that refusal for its activity. Each instance then materialises its own checkout, named from the instance index its delivery already carries, commits there, and reports the branch it made; the activity the fan converges on reconciles the branches the container names. Nothing goes on the collection — a work unit describes work. Session persistence stays at convergence — the record and planning folder are shared however the checkouts are split. Costs a full checkout per branch and makes the convergence responsible for a merge that can conflict, so reach for it only where per-unit attribution is the point. |
