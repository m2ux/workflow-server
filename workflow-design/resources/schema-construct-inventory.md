...
| "subagent-isolation / workers with their own checkout" | **Borrow isolated-fan-out** | Borrow [`meta/patterns/04-isolated-fan-out.yaml`](../../meta/activities/patterns/04-isolated-fan-out.yaml); seed `{isolation_mode}` (`context` \| `worktree`). Completeness gate before synthesise. The workers run one at a time, and this is the only route to a worker-owned worktree: a graph fan's branches share one working tree and one git index, so the load refuses a fanned activity binding any version-control operation. Work that mutates no checkout belongs in the graph fan instead. |
