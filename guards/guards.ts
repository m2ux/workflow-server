...
export const GUARDS: GuardSpec[] = [
  ...
  {
    id: 'identity-binds',
    script: 'guards/check-identity-binds.ts',
    npmScript: 'check:identity-binds',
    scope: 'corpus',
    gatesServing: false,
    json: true,
    proves: 'no activity step restates the same-name binding omitting the pair already performs, so a reader tells a deviation from a no-op without comparing the two sides of a colon',
    form: 'none',
  },
  {
    id: 'inherited-inputs',
  ...
  {
    id: 'corpus-links',
    script: 'guards/check-corpus-links.ts',
    npmScript: 'check:corpus-links',
    scope: 'corpus',
    gatesServing: false,
    json: true,
    proves: 'every reference out of a namespace names the namespace rather than counting directories to it, and every namespace-anchored link lands on a file the corpus holds',
    form: 'none',
  },
  {
    id: 'protocol-shape',
    script: 'guards/check-protocol-shape.ts',
    npmScript: 'check:protocol-shape',
    scope: 'corpus',
    gatesServing: false,
    json: true,
    proves: 'every phase of every procedure is named in a heading of its own, so a reviewer can cite one and a resource can link to one',
    form: 'none',
  },
  {
    id: 'section-framing',
