  ...
  for (const { dir, manifest } of corpusNamespaces(root, index)) {
    ...
    for (const { path } of definitionsUnder(activities)) {
      ...
      for (const m of body.matchAll(MAP_BIND)) {
        ...
        const path = (anchor
          ? namespaceSubdir(index, workflow, join(tail, 'TECHNIQUE.md'))
          : namespaceSubdir(index, workflow, join('resources', `${tail}.md`))) ?? '';
...
if (isMain) {
  ...
  const resolveRoot = () => requireRootOrExit('canonical-home-map', DEFAULT_ROOT);
