 ...
 * against the schemas.
 *
 *   npx tsx guards/validate-workflow-yaml.ts                     # every workflow in the corpus
 *   npx tsx guards/validate-workflow-yaml.ts --root /wt/workflows # every workflow in a worktree
 *   npx tsx guards/validate-workflow-yaml.ts workflows/prism      # one workflow
 *
...
function targets(): { root: string; dirs: string[] } {
  ...
  if (positional) {
    ...
    if (!existsSync(dir)) {
      ...
      console.error('Usage: npx tsx guards/validate-workflow-yaml.ts [<path-to-workflow-dir>] [--root <workflows-dir>]');
      process.exit(2);
    }
