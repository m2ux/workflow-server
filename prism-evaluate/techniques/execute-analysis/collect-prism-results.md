...
Collect the outputs of a completed prism run — the report path, the analysis artifact paths, and the analysis units with their locations — and append them to the evaluation's running accumulators.

## Protocol

### 1. Collect Prism Results

- From the completed prism run for `{current_group}`, capture the report path (`REPORT.md`) when one was generated.
...
- Append the run's reference and artifact paths to `{completed_analyses}` and `{all_artifact_paths}`.
