...
Rebase the security feature branch onto its merge base and re-sign every commit so each carries a valid GPG signature, then verify no unsigned commit remains.
...
Findings produced by the scope and artifact review of the security fix; a re-sign failure is appended as an additional finding.

## Protocol

### 1. Re-sign Commits

- Determine the merge base `{$merge_base}` of `{branch_name}` against the private-fork base.
...
- When a push is required, use `` `git push --force-with-lease` `` to the `security` remote.  
