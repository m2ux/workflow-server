...
description: Each per-submodule scanner agent (S1-Sn) writes a JSON file to the planning folder following this schema.
metadata:
  version: 1.0.0
  order: 4
  legacy_id: 4
...
Where `{scanner_number}` is the scanner agent ordinal (1-based) and `{submodule}` is the submodule directory name (e.g., `s1-midnight-node.json`).
