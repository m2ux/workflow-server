...
| Workflows not found | Readable `--workflow-dir` pointing at a tree discovery can walk: when that tree holds a `corpus/` grouping, discovery walks that grouping and does not search sibling folders; a still-flat tree of workflow directories is walked as the root. Specimen workflows are reached by pointing at `corpus/specimens/`. Authoring docs live at `docs/` of that tree and are not product workflows. |
