...
| Workflows not found | Readable `--workflow-dir` pointing at a tree discovery can walk: product definitions under `corpus/`, or a still-flat tree of workflow directories. Specimen workflows are reached by pointing at `specimens/`. Authoring docs live at `docs/` of that tree and are not product workflows. |
