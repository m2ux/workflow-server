...
- **Split a compound heading before leftover tests.** Catalogue order is not application order for a `## Rules` heading or an `## Inputs` / `## Outputs` entry. Apply Detect to each constraint the heading holds on its own: `one-invariant-per-rule` then `no-rule-protocol-restatement` and `no-one-step-rules` on a rule; `procedure-in-io-contract` and `no-bind-mechanics-as-prose` on an I/O description. Honour those entries' **Do not flag** carve-outs only for the constraint under test. A first sentence that is a genuine invariant or genuine shape does not cover a sibling recovery or restatement clause. A splitter finding, or a rewrite that moved one sentence, does not mutate the rest of the heading; testing the unsplit heading next is the same miss. Fetch each Detect from the catalog.
