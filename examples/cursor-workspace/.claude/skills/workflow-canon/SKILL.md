...
5. **Write into that shape**, then run § Mechanical checks. A `## Rules` heading written or thickened in this pass takes the split-before-restatement walk in § Audit → Walk before restatement is treated as settled.
...
- **Walk entry-major, not file-major.** Take one unit, carry its Detect across the whole surface, and record the paths it covered before taking the next unit. One Detect held against many files is a uniform test; many Detects held against one file decay across the catalogue, and the entry that decays first is the one that already fired somewhere else — its remaining instances then read as absent rather than as unlooked-for. A unit walked this way needs no separate sweep to find its other instances, because they are the rest of its own pass.
- **Split a compound `## Rules` heading before restatement tests.** Catalogue order is not application order for that heading. Apply `one-invariant-per-rule` first, then `no-rule-protocol-restatement` and `no-one-step-rules` to each constraint that split produces. Honour those two **Do not flag** carve-outs only for a constraint that remains after the split. A splitter finding does not mutate the file; testing the unsplit heading next is the same miss. Fetch each Detect from the catalog.
...
- Status `walked` on a unit that intersects the change surface requires field-level evidence on **each whole file** in that intersection the unit can reach — not on the diff hunks alone, and not on the first file that produced a finding. A unit evidenced where it fired and asserted over the rest of its reach is `blocked` for that remainder, and the ledger says over how many paths. A narrative "walked the change" without whole-file evidence is `blocked`.
- **Every path on the worklist takes a disposition too, and a search does not hand one out.** Mark each `read` when its whole contents were inspected and `unread` otherwise. A pattern returns the files it names and no others, so a directory searched is not a directory walked: the files holding what no pattern anticipated are missing from the result and from the walk together, and nothing in the result distinguishes them from files that held nothing. Only reading closes that distance, so a scan earns a finding and never a disposition. Reconcile the two against the worklist per [references/reporting.md](references/reporting.md#file-coverage).
...
Two claim shapes the file cannot settle, however carefully it is re-read: one about how the construct resolves, and one about how widely its form is used. Both are re-derived elsewhere, per the Evidence non-negotiables below.
...
### 8. File what the walk mechanised

A unit whose Detect the walk applied by pattern rather than by judgement is a guard the suite does not have yet. Name each one and what it would key on, and file it against the registry — the pass that hand-walked it is the one holding the evidence that the pattern fires, and that evidence is gone by the next pass.

The threshold is the second occurrence. One instance is an incident; a unit that fired twice in one walk, or once in each of two consecutive walks, is a pattern a check can hold. Leaving it in prose spends every later pass's reading budget on work a guard does in milliseconds, which is the budget those passes then do not spend on the units only judgement can reach. Report the candidates so the number of hand-applied units is visible and falling.
...
- **A withdrawal is a carve-out discovery, and the carve-out is its home.** Where a candidate falls because the entry's Detect does not reach the shape, because the resolver settles it, or because the corpus carries the form as convention, that knowledge is about the canon rather than about this target. Its home is the entry's **Do not flag** or the guard's exemption surface, as an edit raised in the same pass. Recorded only as a register row it has this pass's lifetime, and the next pass pays the same resolver cost to reach the same answer — which is the most expensive knowledge a walk produces, since a withdrawal is usually settled by opening a home the file itself cannot answer from.
...
- **A claim about resolution is settled by the resolver.** That a reference does not resolve, that a declaration is redundant, that a value has no producer — each is a claim about what consumes the construct, and the file holding the construct does not answer it. Name the consumer and derive the finding there: the guard that reads the field, the loader that resolves the reference, the call sites that bind it. Read against the wrong resolver, a well-formed reference and a broken one are the same characters, so the finding survives every re-reading of the file that produced it and falls the first time the consumer is opened.
- **Form is discriminated by the corpus, not by the file.** Where an entry's Detect keys on a form — a string shape, a naming, a field order, a message opener — one construct supports *defect* and *established convention* equally, and only the sibling set tells them apart. Sweep for the form before recording. A form the siblings carry is a convention, and a finding against it is a finding against that convention: a wider claim, argued against the reference workflows rather than against the one file, and answered by changing the convention or by conforming to it.
