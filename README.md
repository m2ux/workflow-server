...
1. **Match the user's goal** to an activity via `get_activities` and then `get_activity`
2. **Determine the skills** needed for the activity via `get_skills` and then `get_skill` to orchestrate the right tool sequence
