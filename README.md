...
2. **Start session** — `start_session` returns a session token; `get_workflow` returns the workflow structure, the workflow's `techniques[]` bundled under `techniques` and `rules`, and the `initialActivity` ID
3. **Navigate** — `next_activity` advances the session to the next activity; `get_activity` returns the activity's full definition (steps, checkpoints, transitions) along with the activity's `techniques[]` bundled under `techniques` and `rules`. `get_resource` lazy-loads reference material referenced by a technique
