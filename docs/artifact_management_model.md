...
2. **Engineering root:** Orchestration metadata — plans, session state, traces, ADRs. Bound as `ServerConfig.engineeringDir`:
   - **Repo layout** (`--repo` / `init-repo.sh`): `$HOST_PROJECTS_ROOT/<repo>/.engineering/` (eng submodule or materialised tree inside the main checkout; basename only).
...
Install layout (HTTP/Docker): [`scripts/install.sh`](../scripts/install.sh) writes `$INSTALL/env` with `HOST_PROJECTS_ROOT` and creates `$INSTALL/{workflows,state}`. Per-repo materialisation: [`scripts/init-repo.sh`](../scripts/init-repo.sh). See [install-projects-worktrees.md](install-projects-worktrees.md).
