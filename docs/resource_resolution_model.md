...
Full delivery on every call is the default: a freshly spawned worker lands in an empty agent context, and the repeated bundle is load-bearing. An agent context that has already received content can opt into reference delivery instead — that content is replaced by a short marker rather than repeated. Reference delivery is a property of the **agent context**, not of the session: a solo walk is one context for the whole walk, and a dispatched worker is one context from spawn until it finishes, across any harness resume in between.

- **Opt-in:** `start_session { context_mode: "persistent" }` (also accepted by `dispatch_child` for the child session), or per call via `bundle: "reference"` on `get_activity`, `get_technique`, or `get_resource`. `full: true` on `get_technique` / `get_resource` overrides the per-call opt-in.
- **Ledger:** the server records a hash of every `get_activity` bundle technique, rules block, `activity_rules` block, full `get_technique` payload, full `get_resource` payload, and `get_workflow` orchestrator ops bundle it delivers (in `session.json#deliveredContent`). Recording happens in every mode, so a per-call `bundle: "reference"` can refer back to content delivered under the default full mode. Content keys are namespaced by delivery channel (`bundle:*`, `technique:*`, `activity_rules:*`, `workflow_bundle:*`, `resource:*`) so a marker only ever references content delivered in that same channel.
...
- **Agent-context scoping:** the ledger is keyed on the **delivery scope** — the per-call `agent_id` when supplied, otherwise the session's recorded `agentId`. A dispatched worker authenticates against the orchestrator's `session_index`, and several workers can hold that index at once, so the scope names the agent context a payload is delivered to rather than the session they share. The orchestrator mints an `agent_id` per dispatch and reuses it verbatim when it resumes that worker: a fresh spawn reads an empty ledger (full delivery), the resumed worker reads its own (references), and a sibling worker is unaffected either way. A marker is only ever valid for the context that received the bytes it stands for. Starting a session under a different `agent_id` likewise begins from an empty ledger.
- **Dispatch accounting:** every `get_activity` records an `activity_dispatched` history event carrying `{ agentId, dispatch: "fresh" | "resume", chars }` and echoes the discriminator on `_meta.dispatch` — derived from whether that scope already has a dispatch event for the activity, so no orchestrator cooperation is required. A dispatch that never calls `get_activity` (an out-of-band worker) records the same event on its first `get_technique` / `get_resource`. Where `activity_usage` counts activity exits, this counts dispatches.
- **Delivery magnitude:** `technique_fetched`, `technique_bundled`, and `resource_fetched` each carry `chars` (the FULL payload size, on both paths) and `delivery: "full" | "unchanged"`, so delivered characters and characters saved by reference delivery are both summable from the ledger.
- **Benchmarks:** `npm run bench:token` compares delivery cost per session mode on a fixed `work-package` walk against the frozen pre-optimisation A0 reference ([`scripts/run-token-benchmark.ts`](../scripts/run-token-benchmark.ts); output includes `vsReference.deliveryCostIndex`, A0 = 100, lower is better). `npm run bench:dispatch` measures the other axis — a fresh worker dispatch against the same worker resumed ([`scripts/run-dispatch-benchmark.ts`](../scripts/run-dispatch-benchmark.ts)). See [development.md](development.md#token-delivery-benchmark).
