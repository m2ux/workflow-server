...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `start_session` | `agent_id`, `workflow_id?`, `working_directory?`, `planning_folder?`, `repo?`, `context_mode?`, `user_request?`, `target_workflow_id?`, `fresh?`, `confirm_host_binding?` | `session_index`, planning path, workflow info, `execution_path`, optional `repo`, optional `client`, or a `decision` with no session | Open or resume a top-level session (default workflow: `meta`). Pass `working_directory` as the checkout under work; the server derives `owner/repo` from that origin. `repo` is optional and must equal the derived origin when present. Named `planning_folder` resumes. A derived dated slug that already holds a session opens the next free numbered folder. A unique catalog match with no resume phrasing also returns `client`. Ambiguous matches, saved-session hits, and basename mismatch return a `decision` (`workflow-selection`, `resume-session`, `host-binding-mismatch`) with no `session_index`. `user_request` seeds the opening request into the variable bag, and children inherit it. `execution_path` is `agent` when a caller walks the definition and `runner` when the server does. [State](state-management-model.md) · [Reference delivery](resource-resolution-model.md#reference-delivery) |
