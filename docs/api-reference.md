...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `get_skills` | `session_token` | Raw TOON skill blocks for the workflow's primary skill | **Deprecated** — prefer `get_workflow`, which now bundles workflow-level operations directly in its response. Retained for workflows still using the legacy `skills.primary` model. Loads the workflow-level primary skill (e.g., the orchestrator management skill). `session_token` authenticates the call and determines which workflow's skill to return. The response is raw TOON content separated by `---` fences, prefixed with scope and session token headers. |
