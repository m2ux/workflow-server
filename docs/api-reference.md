...
| `yield_checkpoint` | `session_token`, `checkpoint_id` | Status, updated `session_token`, and instructions | Yields execution to the orchestrator at a checkpoint step. `session_token` authenticates the call and must have an active activity. `checkpoint_id` identifies the checkpoint to yield (must match a checkpoint defined in the current activity). The returned `session_token` (with its `bcp` field set) is the checkpoint handle the worker must yield to the orchestrator via a `<checkpoint_yield>` block. The status confirms the yield was recorded. **Hard gate:** Cannot yield a new checkpoint while another checkpoint is already active and awaiting resolution. |
...
When a worker encounters a checkpoint step during activity execution, it calls `yield_checkpoint`. This sets the `bcp` (blocking checkpoint) field in the token and returns an advanced `session_token` (the checkpoint handle). **Calling `next_activity` while `bcp` is set produces a hard error** (not a warning). Most other tools are also gated when `bcp` is active.

The worker yields the bcp-bearing `session_token` to the orchestrator inside a `<checkpoint_yield>` block. The orchestrator then calls `present_checkpoint` to load the user-facing details, and `respond_checkpoint` to record the user's choice. With the interceptor installed, the orchestrator's `present_checkpoint` and `respond_checkpoint` calls receive the captured token automatically; the LLM only needs to supply `option_id` (or `auto_advance` / `condition_not_met`):

```json
{ "session_token": "...", "option_id": "proceed" }
```
