...
| `/ready`  | Readiness — 503 when any check below is false                  |
| `/mcp`    | The session: client to server, server to client, and ending it |
...
| ---------------- | ---------- | --------------------------------------------- | -------------------------------------- | ----------------------------------------------------------------------- |
| `discover`       | —          | Server info and a bootstrap stub              | The first call: how to start a session | [Verify](setup.md#3-verify)                                             |
| `list_workflows` | —          | `id`, `title`, `version`, `tags` per workflow | The catalog of available workflows     | [What a namespace is](resource-resolution-model.md#what-a-namespace-is) |
| `health_check`   | —          | Status, version, workflow count, uptime       | Process health                         | [HTTP endpoints](#http-endpoints)                                       |
