...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `start_session` | `agent_id`, `workflow_id?`, `working_directory?`, `planning_folder?`, `repo?`, `context_mode?`, `user_request?` | `session_index`, planning path, workflow info, optional `repo` | Open or resume a top-level session (default workflow: `meta`). Pass `working_directory` as the checkout under work; the server derives `owner/repo` from that origin. `repo` is optional and must equal the derived origin when present. Named `planning_folder` resumes. `user_request` seeds the opening request into the variable bag, and children inherit it. [State](state-management-model.md) · [Reference delivery](resource-resolution-model.md#reference-delivery) |
