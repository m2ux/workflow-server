...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `get_workflow` | `session_index` | Orchestrator technique bundle + workflow stubs | Orchestrator load: rules, variables, `initialActivity`, activity list. The bundle is held to what one tool result may carry (`MAX_RESPONSE_CHARS`): operation bodies ride it in list order until the next would overflow, the rest are named under `operation_refs` and fetched with `get_technique { technique_id }`. The role's `rules` are never bounded. [Resolution](resource-resolution-model.md) |
