...
| `get_activity` | `session_index`, `context_tokens`, `agent_id?`, `bundle?` | Worker bundle + activity body, `exit_destinations`, `_meta.dispatch` | Worker load for the current activity. `context_tokens` is required; `agent_id` scopes delivery to this worker context. `exit_destinations` names the activity each declared exit leads to, so a worker resolves its own routing. [Bundling](resource-resolution-model.md#hybrid-technique-bundling) · [Reference delivery](resource-resolution-model.md#reference-delivery) |
