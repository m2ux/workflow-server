...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `get_workflow` | `session_index` | Orchestrator technique bundle + workflow stubs | Orchestrator load: rules, variables, `initialActivity`, activity list. The bundle is held to what one tool result may carry (`MAX_WORKFLOW_RESPONSE_CHARS`): operation bodies ride it in document order until the next would overflow, the rest are named under `operation_refs` and fetched with `get_technique { technique_id }`. The role's `rules` are never bounded. [Resolution](resource-resolution-model.md) |
...
| `get_activity` | `session_index`, `context_tokens`, `agent_id?`, `activity_id?`, `bundle?` | Worker bundle + activity body, `exit_destinations`, `fan_instance` where the graph runs it over a collection, `_meta.dispatch` | Worker load for the activity this context was dispatched for, named by `activity_id` where several are in flight. `context_tokens` is required; `agent_id` scopes delivery to this worker context. `exit_destinations` names where each declared exit leads, exactly as the graph names it — one activity, a list of members, or one activity together with the collection it runs over — so a worker reports that destination unread. [Bundling](resource-resolution-model.md#hybrid-technique-bundling) · [Reference delivery](resource-resolution-model.md#reference-delivery) |
