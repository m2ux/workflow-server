...
Per-step lazy fetching is the default: a worker loads each technique step's composed content via `get_technique { step_id }` at the moment it executes the step. An activity can opt into hybrid bundling instead by declaring `bundleTechniques: { maxChars: <n> }` — `get_activity` then inlines the composed content of the activity's small, ungated step-bound techniques so those steps execute without a fetch round-trip.

- **What is bundled:** each technique-kind step whose composed wire form is at most `maxChars` characters and that is not gated — a `when`/`condition` on the step itself or on an enclosing loop keeps it lazy, so bundling never delivers content for a step that may not execute. Everything else remains a `get_technique { step_id }` fetch.
- **Response shape:** bundled entries arrive in the ops section under `step_techniques`, keyed by step id. Each entry is the step's full composition — activity-group resolution, ancestor contract, inherited-input blocks, and binding-seam provenance annotations — identical to what the step-bound `get_technique` fetch returns. A `step_techniques_note` states that steps absent from the map still require a fetch, and `_meta.bundled_steps` lists the bundled step ids.
- **Ledger interplay:** bundled entries share the `technique:<resolvedId>` ledger key with `get_technique`, so in a persistent-context session a bundled delivery collapses a later step-bound refetch to an unchanged-reference — and reference-mode re-delivery of the activity collapses already-delivered bundled entries to `{ unchanged: true, content_hash }` markers. `bundle: "full"` re-delivers everything.
...
The flag is a per-activity authoring decision, weighing fetch-round-trip savings against the per-step attention anchor that lazy fetching provides.
