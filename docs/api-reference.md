...
| Tool | Parameters | Returns | Description |
|------|------------|---------|-------------|
| `get_workflow` | `session_index` | Orchestrator technique bundle + workflow stubs | Orchestrator load: rules, variables, `initialActivity`, activity list. The whole result — text and `_meta` together — is held to what one tool result may carry (`MAX_RESPONSE_CHARS`). Operation bodies ride it in list order until the next would overflow, the rest are named under `operation_refs` and fetched with `get_technique { technique_id }`. Where every body is already an id and the response is still over, the prose explaining a variable gives way next: first for the names activities declare, then for all of them, with `variables_note` saying so. The role's `rules` and the declared namespace are never bounded. [Resolution](resource-resolution-model.md) |
