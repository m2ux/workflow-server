...
| `get_workflow_status` | `client_session_token?`, `client_session_id?`, `parent_session_token?` | `status` (active/blocked/completed), `current_activity`, `completed_activities`, `last_checkpoint` info, and variables | Checks the status of a dispatched client workflow session. `client_session_token` directly authenticates the query. Alternatively, provide `client_session_id` together with `parent_session_token` for authorization when the client token is not available. The returned `status` is one of `active`, `blocked`, or `completed`. `current_activity` names the activity the sub-agent is executing. `completed_activities` lists all finished activities. `last_checkpoint` contains the most recent checkpoint details. `variables` reflects the current workflow state. |
...
- **Bootstrap**: `start_session(workflow_id, agent_id)` → `get_skills` → `get_workflow` → `next_activity(initialActivity)` → `get_activity`
