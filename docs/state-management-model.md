...
A write also carries the bytes its call read. The state file is replaced only while it still holds them, and a call whose file has moved on since is refused with `STALE_WRITE` and nothing written; the response says to repeat the call, which then works from the current state. So two calls in flight against one session end with one refused rather than with one of them silently discarded. A parent and its launched children live in a single file — a child's state sits inside its parent's — which is what makes that pairing ordinary rather than exotic: a parent recording a figure while one of its children advances is two writes against the same bytes.
