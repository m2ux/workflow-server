...
| [`workflows/docs/`](https://github.com/m2ux/workflow-server/blob/workflows/docs/README.md) | Authoring contracts on the `workflows` branch: named-root layout, adding a workflow, the technique file contract | Workflow authors |
...
- **Workflow-authoring contracts** → [`workflows/docs/`](https://github.com/m2ux/workflow-server/blob/workflows/docs/README.md) on the `workflows` branch (layout, adding a workflow, [technique protocol](https://github.com/m2ux/workflow-server/blob/workflows/docs/technique-protocol-specification.md)); the [schema guide](../schemas/README.md) on this tree for workflow and activity files.
...
- **Section headings are sentence case and carry no numbers.** A reader cites a section by its title, so the anchor stays readable and survives a reordering. Document titles spell out "and" rather than using an ampersand. The two normative specifications are the exception: a numbered clause is how a reader cites a contract term, so the [technique protocol specification](https://github.com/m2ux/workflow-server/blob/workflows/docs/technique-protocol-specification.md) and the [orchestra specification](orchestra-specification.md) keep their numbering.
