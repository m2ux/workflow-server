...
At a 200,000-token window, giving a 280,000-character budget, **the cap binds first on measured content**. The benchmark's three activities cost 157,645 characters batched — 57,290, then 54,887, then 45,891 — which is 56% of budget, because a batch's second and later activities collapse the invariant blocks and the ancestor contract their techniques share (see [Reference delivery](resource-resolution-model.md#reference-delivery)). Standalone, the same three cost 167,787, so batching saves 6%. Reaching the budget takes roughly five activities of that weight, and a worker declaring a smaller window is bounded proportionally: the budget takes over below roughly 113,000 declared tokens on this workload.

Every delivery is also held to what one tool result may carry, and that is the limit shaping these figures: no activity of this run reaches the budget's share of a window before it reaches that bound, so the deliveries that have more to give than the bound admits sit just under it rather than under anything the batch policy sets, and the rest sit wherever their own content ends. It is also why a batch saves so much less in characters than it once did. Both arms now sit under one ceiling, and what a collapse frees inside a bounded response is spent on step techniques the worker would otherwise fetch lazily — so characters leave this measurement, which counts activity payloads only, and reappear as round trips it never counted. What a batch buys is the dispatches it does not pay for and the lazy fetches its fuller deliveries avoid, rather than a smaller total on the wire.

These figures come from `npm run bench:batch` against the corpus at `5fa925ae`. They move whenever an activity's payload does, so re-run it rather than trusting the numbers here; the shape of the claim — cap before budget, at this window — is what the paragraph is for.
...
`get_activity` reports where a context stands in `_meta.batch` (`activities_delivered`, `max_activities`, `delivered_chars`, `budget_chars`, `may_continue`), so the ordinary end of a batch is the worker stopping. The count is of activities **delivered** to that context, which is the only one the session can answer — a delivery is the sole record that an activity reached a context, so one advanced onto an activity whose body it never fetched leaves nothing to count — and the only one both limits are about, since each protects what a context is holding. Asking past the bound is refused with the payload undelivered and a `batch_refused` history event naming the limit — recorded once per scope, activity and limit, so the tally counts how often a limit bound rather than how often a worker retried. That tally is what the starting settings are revised from.
