...
The system enforces strict boundaries between orchestration metadata (the workflow engine, plans, and state) and the user's domain codebase. Planning and session files live under the **engineering root** (`ServerConfig.engineeringDir`); feature worktrees live under the **worktree root** (`ServerConfig.workspaceDir`). With `--repo=owner/repo` these are `$HOST_PROJECTS_ROOT/<repo>/.engineering` and `$HOST_PROJECTS_ROOT/<repo>/.worktrees` (basename checkout **you** keep under the projects root; engineering via [`scripts/deploy.sh`](../scripts/deploy.sh) in that project). See [install-projects-worktrees.md](install-projects-worktrees.md). `init-repo.sh` is deprecated. Legacy single-root mode still uses a nested `.engineering/` tree under one workspace path. This document covers planning-folder layout, `README.md` progress tracking, `artifactPrefix` naming, the server-computed `artifacts` contract, and git procedures for engineering checkouts.
