...
The grain of the work — judgement on live feedback versus accepted, codified application — is [A Technique Is a Reading](/canon/resources/design-principles.md#26-a-technique-is-a-reading) and [A Routine Holds the Codified Path](/canon/resources/design-principles.md#42-a-routine-holds-the-codified-path). This section is the file contract.
...
The file carries `id` (matching the filename), `version`, `name`, `description`, optional `inputs` / `outputs` / `internals`, and `steps[]` as the ordinary kind-tagged list. It declares no `exits`, no `outcome`, no `rules` and no activity-wide `techniques`. How those fields map, and when a path lives here rather than as a technique, is the [schema construct inventory](/canon/resources/schema-construct-inventory.md#routine-level-constructs-routineschemajson) and [A Routine Holds the Codified Path](/canon/resources/design-principles.md#42-a-routine-holds-the-codified-path).
