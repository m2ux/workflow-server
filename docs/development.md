...
| `workflows/` | A worktree of the `workflows` branch. Product definitions live under `corpus/`; discovery walks that grouping and does not search sibling folders. A directory holding `techniques/`, `resources/` or `routines/` at any depth under `corpus/` is a namespace references can name, and one holding a `workflow.yaml` is a workflow as well; grouping folders organise the tree and name nothing. A namespace answers to its directory name, and to the path from `corpus/` that reaches it. Layout authoring lives at `workflows/docs/`. |
