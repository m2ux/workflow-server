...
describe('dispatch_workflow tool: session creation', () => {
  it('creates a client session token with psid referencing the parent', async () => {
    const parentToken = await createSessionToken('meta', '1.0.0');
    const parent = await decodeSessionToken(parentToken);

    const clientToken = await createSessionToken('remediate-vuln', '1.2.0', parent.sid);
...
describe('parent-child session correlation', () => {
  it('parent can find children via psid', async () => {
    ...
    const child1Token = await createSessionToken('remediate-vuln', '1.2.0', parent.sid);
    const child1 = await decodeSessionToken(child1Token);

    const child2Token = await createSessionToken('work-package', '1.0.0', parent.sid);
  ...
  it('psid does not grant access to parent session', async () => {
    ...
    const childToken = await createSessionToken('remediate-vuln', '1.2.0', parent.sid);
  ...
  it('recursive dispatch: child can be a parent too', async () => {
    const metaToken = await createSessionToken('meta', '1.0.0');
    const meta = await decodeSessionToken(metaToken);

    const clientToken = await createSessionToken('remediate-vuln', '1.2.0', meta.sid);
    const client = await decodeSessionToken(clientToken);

    const subClientToken = await createSessionToken('prism-update', '1.0.0', client.sid);
