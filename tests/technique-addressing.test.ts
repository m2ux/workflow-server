...
import { resolveOperations } from '../src/loaders/technique-loader.js';
...
describe('technique addressing (:: path)', () => {
  it('resolves an implicit same-workflow sub-technique', async () => {
    const [r] = await resolveOperations(['cargo-operations::run-suite'], WF_DIR, 'work-package');
    expect(r.type).toBe('operation');
  ...
  it('resolves the full canonical path workflow::technique::sub', async () => {
    const [r] = await resolveOperations(['work-package::cargo-operations::run-suite'], WF_DIR, 'meta');
    expect(r.type).toBe('operation');
  ...
  it('delivers a whole (standalone) technique protocol from a bare ref', async () => {
    const [r] = await resolveOperations(['classify-problem'], WF_DIR, 'work-package');
  ...
  it('resolves a cross-workflow ref (legacy / and :: forms equivalently)', async () => {
    const [slash] = await resolveOperations(['prism/portfolio-analysis'], WF_DIR, 'work-package');
    const [colons] = await resolveOperations(['prism::portfolio-analysis'], WF_DIR, 'work-package');
  ...
  it('group-prefix rule ref still expands (agent-conduct::checkpoint-discipline)', async () => {
    const resolved = await resolveOperations(['agent-conduct::checkpoint-discipline'], WF_DIR, 'work-package');
