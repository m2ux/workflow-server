...
| "Two activities run the same sequence" | **Routine reference** | `steps[]` entry with `kind: routine`, `.id`, `.routine` naming `[workflow::]name`, `.with` binding each routine input id to the value it takes here, and `.outputs` naming the session variable each routine output lands under. The sequence itself is a file under `routines/` carrying its own signature — inputs, outputs and steps — so it is checkable with no host activity, and a reference is spliced away when definitions load. A declared input left unbound takes the host's value under the input's own id. |
...
| "Use get_workflow to load data" | **Protocol step** | the call is written in the phase that makes it, arguments and all — the harness puts the tool's own schema in front of the caller, so the phase says which arguments it passes and what it does with the response, never what an argument is (`no-tool-schema-restatement`) |
