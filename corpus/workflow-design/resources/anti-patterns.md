...
**Do not flag:** Correct dotted ancestry address — `[<workflow>.]<technique>.<rule-name>` (e.g. `meta.gitnexus-operations.index-freshness-first`). Shorten when in ancestry: omit workflow for same-workflow; bare rule name when inherited from self/group/workflow root. Full path only for rules outside current ancestry. A hyperlink from a surface the merge does not reach — a resource, a README, or a technique under a different container — where the reader holds none of the cited rules and the link is navigation rather than a citation in the wrong form. Test: resolve the citing file against the declaring container; inheritance applies only where the citer sits beneath it.
