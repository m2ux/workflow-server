...
**Do not flag:** Correct dotted ancestry address — `[<workflow>.]<technique>.<rule-name>` (e.g. `meta.gitnexus-operations.index-freshness-first`). Shorten when in ancestry: omit workflow for same-workflow; bare rule name when inherited from self/group/workflow root. Full path only for rules outside current ancestry. A hyperlink from a surface that runs no Protocol — a resource or a README — which names a rule to send a reader to it rather than to honour it. A technique citing any rule takes the dotted address at the length its ancestry allows, whether or not the rule is one it inherits. Test: ask whether the citing file is a technique; if it is, the form is dotted.
...
The heading is a name, not the instruction's opening words repeated above it. "Verify dispatch" over a bullet confirming an id is a name; "Branch on the response status" over a bullet reading *Branch on the response `status`* is the same words twice, and a reader scanning headings for the shape of the work learns nothing the bodies were not already going to tell them. Write the result in the shortest terms that distinguish this phase from its neighbours, and let the bullet say how. A heading that can only be written by copying its bullet is usually a phase that has no outcome of its own, which is the disposal above.
