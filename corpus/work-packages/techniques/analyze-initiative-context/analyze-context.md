...
- Ground the shared-dependency and common-infrastructure rows in the graph rather than in the packages' descriptions of themselves. Apply [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[resolve-graph](/gitnexus/techniques/resolve-graph.md)(*tree_path*: the initiative's codebase) for the `{repo_name}` the reads below address, then [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[query](/gitnexus/techniques/query.md)(*search_query*: each package's concept as keywords, *repo_name*: `{repo_name}`) and read its `{query_report}` for the execution flows the package lands in — two packages whose reports name the same flow share infrastructure whether or not either says so
- Where a flow is common to two packages, apply [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[impact](/gitnexus/techniques/impact.md)(*target*: the shared symbol, *direction*: `upstream`, *repo_name*: `{repo_name}`) and read its `{impact_report}` as the ordering constraint between them — the package that changes the symbol precedes the package that reads it
