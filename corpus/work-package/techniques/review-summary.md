...
- Apply [view-pr](/github/techniques/view-pr.md)(*repo_path*=`{host_repo_path}`) and read the pull request's merge state before composing anything.
...
- Resolve `{$reviewed_code_base}`: `{reviewed_code_base_url}` when it is non-empty; otherwise Apply [view-pr](/github/techniques/view-pr.md)(*repo_path*=`{component_git_dir}`) and take `{reviewed_code_base_url}` from the op.
