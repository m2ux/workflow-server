...
4. Apply [post-pr-review](/github/techniques/post-pr-review.md)(*repo_path*=`{component_git_dir}`, *body*=`{review_summary}`, *review_event*=`{$review_event}`); set `{review_posted}` and `{posted_review_id}` from the op. This is a pull-request review, not a description body update ([render](./render.md)).
