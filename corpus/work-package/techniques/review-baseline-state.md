...
- Apply [view-pr](/github/techniques/view-pr.md)(*repo_path*=`{component_git_dir}`); set `{base_branch}` from the op output.
...
- Apply [list-pr-files](/github/techniques/list-pr-files.md)(*repo_path*=`{component_git_dir}`); set `{changed_files}` from the op output.
