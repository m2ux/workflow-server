...
- Create `{token_usage_document}` at `{planning_folder_path}` per manage-artifacts.artifact-prefix, following [token-usage](/meta/resources/token-usage.md#template) and its [Rules](/meta/resources/token-usage.md#rules), filling it from the per-activity figures and `{token_usage_document.usage_coverage}`
