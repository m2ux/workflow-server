...
The run is `gitnexus::orphan-scan`, which supplies a literal graph query for every unreferenced function and method and narrows it to the files a change touched. The positive case names two files the graph holds, so the narrowing keeps rows and the run lands whatever nothing references among them. The negative case names a file no graph holds, so the narrowing keeps no row and the run lands an empty set — which is the fallback the run promises for a file outside the index, and which the report names as such rather than as a clean file.
...
|---|---|---|
| `positive-case` | `orphan-scan` | files the graph holds |
