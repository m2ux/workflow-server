...
Refers to the gitnexus library's [`group-radius`](/gitnexus/routines/group-radius.yaml) run via [`01-measure-reach.yaml`](./01-measure-reach.yaml). The group, the concern and its home graph travel under the run's own names, and the six values the run produces are bound under their own names too. Leads to [Report Conformance](#02-report-conformance).
