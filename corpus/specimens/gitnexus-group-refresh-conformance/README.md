...
The run is `gitnexus::group-refresh`, which reads how far every member of a repository group has fallen behind its code, rebuilds each member the report names, rebuilds the contract registry those members' moves left behind, and reads what the rebuilt registry holds. The positive case prepares a throwaway group of two checkouts, one standing a commit behind its graph, so the freshness read names that member, the rebuild loop runs a pass over it, and the run lands an empty `unrebuildable_members` beside the rebuilt registry's counts and contracts. A group someone configured is what a walk would otherwise rebuild and rewrite. The negative case binds the `midnight-ledger` group, whose member `midnight-node` has no graph, so the run lands a non-empty `unrebuildable_members` naming that member while it still rebuilds the registry over the members it can read — which is the fallback the run promises for a member with no graph, and which the report names as such rather than as a member the freshness report found current or as a refresh that failed.
...
|---|---|---|
| `positive-case` | `group-refresh` | a prepared group with one member behind its graph |
