...
The union case carries a two-arm `UNION ALL` query — one arm for every function, one for every method, each returning the symbol's name and file — over two files the graph records **both** kinds in. The run's composition step names this shape: each arm is its own `MATCH`, and each arm takes its own path predicate, so the rows that land are functions from the first arm and methods from the second, drawn from both files and from nothing outside them. The report names that mark as the two-arm shape, so a predicate riding two arms reads apart from the positive case's predicate on one.

The files are chosen for holding both kinds. Over files of functions alone the second arm keeps nothing, and the case evidences the composition running rather than the predicate reaching every arm.
...
| `union-case` | `narrow-to-changed` | a two-arm function-and-method query over two files holding both kinds |
