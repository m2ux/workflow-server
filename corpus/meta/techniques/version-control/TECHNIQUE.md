...
Host repository and component under work are two facts, never one variable. The session bind is the origin of the checkout passed as `working_directory` on `start_session`. After that bind exists, [resolve-host-repo](./resolve-host-repo.md) takes the host path — the outermost superproject that claims the workspace checkout — and lands as `{host_repo_path}` and `{target_repo}`. A repository named in the user's request, such as the `owner/repo` in a PR or issue URL, identifies the COMPONENT and lands as `{mentioned_repo}`; it is component context for [select-target-component](./select-target-component.md) alone and never substitutes for the bound host. Treating a prose-named repository as the host is what produces a checkout that does not exist.
