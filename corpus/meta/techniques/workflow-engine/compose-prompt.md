...
- When `{agent_technique}` is [workflow-orchestrator](./workflow-orchestrator.md): instruct `get_workflow { session_index }`, which delivers the operations bundle step 3 directs the agent to Apply. The session the orchestrator is spawned for is already open, and its index is bound above
