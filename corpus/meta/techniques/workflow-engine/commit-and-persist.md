...
5. Confirm the engineering push landed (remote tracking branch includes the new commit). If push failed, retry once; if still failing, surface the error and do not advance to the next activity.
6. Emit the run status, filling the [Template](/meta/resources/run-status.md#template) and honouring the [Rules](/meta/resources/run-status.md#rules) beneath it. This is the last phase, after the push is confirmed, so every link the emission publishes points at an artifact the remote already holds.
