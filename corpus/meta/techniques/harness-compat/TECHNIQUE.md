...
Abstract sub-agent dispatch techniques — harness-independent vocabulary for spawning, continuing, and concurrently dispatching agents.
...
All techniques and activities MUST reference technique names from this technique ([spawn-agent](./spawn-agent.md), [continue-agent](./continue-agent.md), [spawn-concurrent](./spawn-concurrent.md)) rather than harness-specific tool syntax. Harness-specific invocation details live only in the harness technique files ([claude-code](./claude-code.md), [cursor](./cursor.md), [cline](./cline.md), [generic](./generic.md)). Generic spawn/continue/concurrent ops resolve the target through [resolve-harness-operation](./resolve-harness-operation.md) — the single authoritative `{harness_kind}` → file map — and must not inline that map.

### foreground-always

CRITICAL: every dispatch technique in this technique MUST be blocking-equivalent for the orchestrator — the orchestrator must observe the worker's `<checkpoint_yield>` or completion before continuing.
