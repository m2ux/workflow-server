...
- Prefer a host resume primitive for `{agent_id}` when available; include `{session_index}` in the prompt (`index-in-prompt`).
...
- Wait (block or completion signal) until the resumed agent yields or completes before continuing. Where the resume primitive returns as soon as the resume is accepted, the wait is the host's completion signal for that agent, held open per `foreground-always`.
