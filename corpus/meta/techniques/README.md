...
Techniques in this library are bound by the meta activities and are also referenced from other workflows, which qualify them by group (`workflow-engine::dispatch-activity`, `knowledge-base-search::catalog-search`). A reference from outside meta is always written qualified.

Cross-cutting conduct rules live in three homes, split by the audience that can act on them: [`agent-conduct`](./agent-conduct.md) holds what binds any agent in a run, [`orchestrator-conduct`](./orchestrator-conduct.md) holds the boundaries only an orchestrator can honour, and [`worker-conduct`](./worker-conduct.md) those only a dispatched worker can. Capability groups reference them as their single source of truth rather than restating them, and a role's bundle addresses the families it owns rather than a whole conduct technique.
