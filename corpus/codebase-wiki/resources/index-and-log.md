...
| When | Technique | Pages | Baseline | Trigger |
|------|-----------|-------|----------|---------|
...
- **The log is append-only.** A new entry is added per technique, in technique order. A prior entry is never edited, so the ledger stays a faithful history.
...
- **Line budget:** one line per page in the index and one row per technique in the log. Both grow with the wiki, so the budget is per entry rather than per file: an index entry over one line, or a log entry over one row, is over budget.
