...
- Apply [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[verify-index](/gitnexus/techniques/verify-index.md) against `{repo_name}` and read its `{index_stale}`, per `index-freshness-first` — a graph built before `{raw_baseline_commit}` describes a tree the citations do not point at.
