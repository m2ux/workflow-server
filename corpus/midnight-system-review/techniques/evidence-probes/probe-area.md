...
- Emit this area's record as `{area_evidence_records}`. One area, one record: the graph runs this technique once per area and lands each record in a slot of its own, so nothing here reads or extends what another area produced.
