...
| **Artifacts** | Output files each activity leaves behind, each named by its producing technique's declared artifact | What durable outputs does a run leave? Which one is a run's terminal record? |
...
| **Techniques** | Capability description and binding sites; reuse shared and cross-workflow techniques before authoring a local one | What techniques do steps perform? Does an existing shared technique already cover it? What is genuinely new? |
