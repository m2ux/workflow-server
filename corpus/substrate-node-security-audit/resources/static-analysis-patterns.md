...
description: A catalog of grep patterns and mechanical verification checks for security auditing Rust/Substrate node codebases.
metadata:
  order: 5
  legacy_id: 5
---
...
grep is the instrument for pattern-*presence* lead generation — matching a literal/regex to surface candidate sites (every entry under Grep Patterns). Where a check's *verification* step is structural — symbol/definition enumeration, call graph, caller/callee, reachability, or cross-function comparison — grep still generates the leads, but the verification routes through the `gitnexus` group when the target is GitNexus-indexed (`gitnexus_available`), per `gitnexus.query-not-grep`; the [search-pattern-catalog](../techniques/search-pattern-catalog.md) technique dispatches that routing. When the index is absent or stale, the manual verification procedure stands. Reading full function bodies remains the instrument for pattern-*absence* findings — the graph does not replace it.
