...
> When `{gitnexus_available}`, take the authoritative denominator of the coverage table — the complete in-scope file (and priority-1/2 symbol) set — from the GitNexus inventory rather than a hand-built `find` listing, so no in-scope file can be silently absent from the gate. Reachability reconciliation of a flagged file to its callers uses [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[context](/gitnexus/techniques/context.md)(*name*: the flagged file's primary symbol, *repo_name*: `{repo_name}`) and [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[impact](/gitnexus/techniques/impact.md)(*target*: that symbol, *direction*: `upstream`, *repo_name*: `{repo_name}`). The gate still turns on a §3-applying agent having READ the body; the graph settles the denominator rather than the read.
...
Every verification protocol is executed mechanically, with no protocol skipped or summarized.
