...
- For each pattern, determine the execution type from the resource entry. Grep-based (PRESENCE) patterns: run the search string against the `{search_scope}`, omitting any paths or file patterns named in `{exclusions}` (e.g., test, mock, bench files). Verification-based patterns (mechanical checks): run the search string, then apply the verification procedure defined in the resource (e.g., trace a variable, check field completeness, verify pairing). Record every hit with file path and line number.
- For a mechanical check whose entry marks the verification step STRUCTURAL (call-graph, reachability, cross-function, or symbol/definition enumeration), the grep search string still generates the candidate leads, but resolve the structural verification through the `gitnexus` techniques when `{gitnexus_available}` — [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[context](/gitnexus/techniques/context.md)(*name*: the symbol the lead names, *repo_name*: `{repo_name}`) for callers/callees, [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[impact](/gitnexus/techniques/impact.md)(*target*: that symbol, *direction*: `upstream`, *repo_name*: `{repo_name}`) for reachability, [gitnexus](/gitnexus/techniques/TECHNIQUE.md)::[cypher](/gitnexus/techniques/cypher.md)(*cypher_query*: a match spelling the check's own structural assertion, *repo_name*: `{repo_name}`) for custom traces and enumeration — rather than tracing by manual read. When not indexed, the manual verification procedure stands.
