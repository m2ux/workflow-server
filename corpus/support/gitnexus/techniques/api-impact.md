...
1. Call `gitnexus_api_impact { route: route_path, file: handler_file, repo: repo_name }` and record the `{api_impact_report}`.
...
2. Read a mismatch as a consumer that will break on the change rather than as one already broken: it reads a key the route does not return today, which a rename or a reshape is about to make true of more of them.
...
3. Read the risk level as the consumer count and the mismatches together — a route nothing outside the module fetches is a different change from one a dozen components read.
