...
A **library** is a namespace of shared techniques that declares no workflow of its own. It belongs to no single product and to every product that reaches it, which is what puts it here at the corpus root.
...
`<namespace>/techniques/TECHNIQUE.md` is the namespace root index, holding the inputs a library's techniques share and the rules the whole library is held to. Each `<op>.md` beside it is one technique, addressed `<namespace>::<op>`. How a base contract reaches the techniques beneath it is defined in [workflow-canonical](/meta/resources/workflow-canonical.md#base-contract-inheritance). A `routines/` directory beside `techniques/` holds named runs of those techniques, addressed `<namespace>::<name>`. The grain between a technique and a run is [A Routine Holds the Codified Path](/canon/resources/design-principles.md#42-a-routine-holds-the-codified-path).

A library declares no `workflow.yaml`, so discovery offers none of these to an operator and no coverage walk enters one. What measures them is the reference guards — a technique nothing binds, an input nothing produces, a rule cited in prose — and the drift walk over every workflow that binds them.
