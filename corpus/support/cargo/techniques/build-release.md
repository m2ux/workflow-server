...
### 1. Compile Release Artifacts

- `{build_budget} cargo build --release {build_scope} {features}`
  ...
  > - Below the host floor `resource-budget` names, run `-p <crate>` for the binary first, then a separate workspace pass for the second product.