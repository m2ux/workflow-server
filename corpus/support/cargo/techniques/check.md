...
- Run `{generated_product_skip} {build_budget} cargo check {build_scope} {features}`, capturing its combined stdout/stderr as `{check_diagnostics}`.
...
### 2. Compose Check Status

- Compose `{check_status}` = `{ check_id: 'check', passed: <exit code 0>, diagnostics: {check_diagnostics} }`.