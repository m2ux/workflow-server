...
- Run the test suite, capturing its per-test failure detail as `{failures}`.
  > - Where `.config/nextest.toml` is present the run goes through nextest, which isolates failures into separate processes, so the peak is lower and the report names which test failed: `{generated_product_skip} RUST_TEST_THREADS=${RUST_TEST_THREADS:-4} {build_budget} cargo nextest run {build_scope} {features} --test-threads=${RUST_TEST_THREADS:-4} {test_filter}`
...
### 2. Compose Test Status

- Compose `{test_status}` = `{ check_id: 'test', passed: <every test passed>, diagnostics: {failures} }`.
