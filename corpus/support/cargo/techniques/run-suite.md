...
- Wait for all four to finish before composing results, collecting every per-op status and its diagnostics.
...
- Compose `{validation_results}` from `{check_status}`, `{clippy_status}`, `{test_status}` and `{fmt_status}`, each of its own fields per that field's Output criteria.