...
A rule name is reached the same way and travels differently. The loader merges a container's rules into every technique beneath it, so a slug names something the reader holds exactly where that container is one the citer sits beneath. Shortened past that, the citation is well-formed, resolves in the checkout, and names nothing that arrived — the dead end an operation reference makes, reached by a shorter road. Ancestry is therefore what sets a rule citation's length, and a rule outside it is named at full dotted address or not named at all.
