...
Also flag an address shortened past what the citer's delivery carries. For each rule a technique names, resolve the slug against the rules that arrive with it — its own, every container it sits beneath, and the contracts of the scopes its role's bundle names, which bring a sibling operation's rules without the citer sitting under it. A bare slug declared only outside that set is a dead end of the kind `unreachable-operation-reference` describes: correctly spelled, declared somewhere, and naming nothing the reader was delivered. Which roles serve a technique is not readable from the tree, so the library is the bound a static walk can prove — within one library a rule may arrive by a route the walk cannot see, and from another it never does. The two lengths fail differently and the shorter one is the one that costs a reader something, so a walk that only measures link spellings settles neither.
...
**Do not flag:** A reference that invokes nothing (`unreachable-operation-reference`); citing resources (including creation-guide Templates); container I/O and rule merge; activity `steps[]` and routine technique binds; activity borrow/bind/include of reusable orchestration patterns; tools; a single capability whose protocol phases are facets of one produce path over tools and resources (load → derive → persist *one* product bag) with no Apply/`::` work invoke; stage/gate locus without an op inventory (`technique-stage-agnostic`).
...
**Detect:** For each value a technique's `## Outputs` enumerates — on an entry, a component, or a nested field — name the phase that leaves it standing when the run ends. Flag a value whose assigning phase is followed by a phase that reassigns the same field and runs over a population including the subjects that value describes, and a value no phase assigns at all. Test: describe the subject carrying this value as the last phase completes; where a later phase's own criteria settle that subject's field, the value is undeliverable however precisely the contract defines it. An early phase recording a value and a late phase settling the whole field read as a sequence, so each phase is correct alone and only their order is wrong.

**Do not flag:** A value a later phase's criteria exclude, by a condition naming the subjects the earlier phase settled. A value the tool supplies in its own response rather than the Protocol deriving it. A value the contract admits and no available fixture exercises, which is a gap in evidence rather than in the definition — the walk records it unevidenced and the declaration stands. A value set whose only home is prose, which is `value-set-in-prose`.

**Fix:** Scope the settling phase to the subjects it judges, so a value an earlier phase recorded survives the phases after it. Where no path can produce the value, delete it from the contract and fold whatever it distinguished into a value that has one — an admitted value nothing produces is a reading a consumer will branch on and never take. See [Separate Contract from Procedure](./design-principles.md#13-separate-contract-from-procedure); the placement defect on a whole output is `output-without-destination`.
