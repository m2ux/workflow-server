...
Also flag an address shortened past what the citer's ancestry supplies. For each rule a technique names, resolve the slug against the rules the loader merges into that technique — its own, and every container it sits beneath. A bare slug declared only outside that set is a dead end of the kind `unreachable-operation-reference` describes: correctly spelled, declared somewhere, and naming nothing the reader was delivered. The two lengths fail differently and the shorter one is the one that costs a reader something, so a walk that only measures link spellings settles neither.
