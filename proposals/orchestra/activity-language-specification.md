# Orchestra: an activity control-flow language

Orchestra proposes writing an activity's control flow into the definition file — its steps, its branch points, its loops — so the path through an activity is legible in the file rather than reconstructed by an agent reading it.

The proposal was specified and not adopted. The server loads a different activity shape: a list of steps, each tagged with the kind of work it does, followed by the named outcomes the activity can reach. Where each outcome leads is the workflow file's to state. There are no flow declarations and no `skill:` key, and the grammar below assumes both.

One primitive is worked out here in full — the activity. What follows is its rationale, a formal grammar, the semantic constraints, a worked example, and the validation rules. The workflow, technique and resource primitives were left unspecified.

The language the server does load is documented in the [schema guide](https://github.com/m2ux/workflow-server/blob/main/schemas/README.md) and the [technique protocol specification](https://github.com/m2ux/workflow-server/blob/main/docs/technique-protocol-specification.md).
...
The complete formal grammar for an Orchestra activity file. The same grammar is kept as a machine-readable file at [`grammar/activity.ebnf`](https://github.com/m2ux/workflow-server/blob/main/grammar/activity.ebnf).
...
The constraints below are also written as an Alloy model at [`constraints/activity.als`](https://github.com/m2ux/workflow-server/blob/main/constraints/activity.als), which is what makes them machine-checkable.
...
The design does not cover the technique primitive, and it does not need to: techniques have a specification of their own in [technique-protocol-specification.md](https://github.com/m2ux/workflow-server/blob/main/docs/technique-protocol-specification.md), and `schemas/technique.schema.json` carries the shape the server loads. A technique is authored as markdown. A technique can contain nested techniques, and there is one kind of technique throughout. Its anatomy is:
...
See [Technique and resource resolution](https://github.com/m2ux/workflow-server/blob/main/docs/resource-resolution-model.md) for resolution semantics.

---

## 5. Resource

The design does not cover the resource primitive — the reference material a technique links to. How the server resolves and delivers resources is in [resource-resolution-model.md](https://github.com/m2ux/workflow-server/blob/main/docs/resource-resolution-model.md).
