...
- **Engine CI prices a fixture walk.** Delivery cost is a property of a walk rather than of any one file, so no guard reads it: `verify.yml` walks `delivery-fixture` under `tests/fixtures/token-bench` and compares the total against `tests/fixtures/token-benchmark-baseline.json`, failing past 1%. Run it the way that job does — `WORKFLOWS_DIR=tests/fixtures/token-bench npm run --silent bench:token -- --workflow=delivery-fixture --label=ci --context-mode=fresh --gate --reference=tests/fixtures/token-benchmark-baseline.json`.
