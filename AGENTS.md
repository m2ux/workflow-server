...
- **Guarding a corpus worktree:** `npx tsx guards/check-all.ts --root <path-to-worktree> --corpus-only`. A worktree of the corpus is a checkout of `workflows` alone — no `package.json`, so `worktree:provision` does not apply to it — and a run without `--root` measures `.worktrees/workflows` of the primary checkout. Point `--root` at the branch root: discovery walks `corpus/` and does not search sibling folders. Specimens under `corpus/specimens/` are reached by pointing there.
