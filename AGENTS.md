...
- **Guarding a corpus worktree:** `npx tsx guards/check-all.ts --root <path-to-worktree> --corpus-only`. A worktree of the corpus is a checkout of `workflows` alone — no `package.json`, so `worktree:provision` does not apply to it — and a run without `--root` measures `workflows/` in this checkout and reports the edits clean without ever reading them. Point `--root` at the branch root: discovery walks `corpus/` and does not search sibling folders. Specimens under `corpus/specimens/` are reached by pointing there.
...
- **Worktree setup:** `npm run worktree:provision` — adds a `workflows` worktree and makes `node_modules` resolvable, so guards and tests measure the worktree you are editing
...
- After code or schema changes, run `npm run typecheck` and `npm test` before committing. Both pass with no `workflows/` directory: live-corpus tests skip when the root is missing or empty.
...
- **Definition changes land on the `workflows` branch.** Walk baselines under `walks/`, the corpus stamp (`npm run baseline:stamp`) and every triage entry the definitions settle move together on that branch. The ledger is what forces this: a definition change that closes a binding finding leaves its entry matching nothing, so a stamp bump alone reports a stale entry, and deleting the entry alone leaves the finding untriaged. The two failures point opposite ways, and no ordering inside the commit avoids both.
- **Engine CI prices a fixture walk.** Delivery cost is a property of a walk rather than of any one file, so no guard reads it: `verify.yml` walks `delivery-fixture` under `tests/fixtures/token-bench` and compares the total against `tests/fixtures/token-benchmark-baseline.json`, failing past 1%. Run it the way that job does — `WORKFLOWS_DIR=tests/fixtures/token-bench npm run --silent bench:token -- --workflow=delivery-fixture --label=ci --context-mode=fresh --gate --reference=tests/fixtures/token-benchmark-baseline.json`. A character-count of the shipped `work-package` walk lives on the corpus tree under `walks/`. The guard sweep and the coverage walk of this corpus run on the `workflows` branch, borrowing this tree's tooling. A change to how walking works still runs the full coverage roster from this tree against the `workflows` branch tip.
