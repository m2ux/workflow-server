...
- **Guarding a corpus worktree:** `npx tsx guards/check-all.ts --root <path-to-worktree> --corpus-only`. A worktree of the corpus is a checkout of `workflows` alone — no `package.json`, so `worktree:provision` does not apply to it — and a run without `--root` measures `workflows/` in this checkout and reports the edits clean without ever reading them. Point `--root` at the branch root: discovery enters `corpus/` and skips `ledgers/`, `walks/` and `docs/` there, and any directory named `specimens` at any depth.
