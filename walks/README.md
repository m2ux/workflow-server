...
A walk drives these definitions through the real server and records what it saw. Those records live here so a later walk compares against this tree rather than against a copy on `main`. The snapshot walks and the coverage walk run on pull requests to this branch.
...
Open the directory for the files as they stand. The walker, the policies and the snapshot tests live on `main` under `tests/e2e/`. They read this folder of the tree they were pointed at.
