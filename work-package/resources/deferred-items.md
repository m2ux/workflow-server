...
- **A raised row names its issue** — the Follow-up cell carries the tracker issue raised for that row, and that issue carries the row ID, so each names the other. A dash means the row is unraised and still open for a later package to take up; a row is raised at most once.
