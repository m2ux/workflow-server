...
3. Resolve link URLs from git remotes — NEVER guess or infer repository URLs, issue numbers, or branch names:
   - Resolve `{$eng_git_dir}`: `{host_repo_path}/.engineering` when that path is a git checkout; otherwise `{host_repo_path}`.
...
5. Update the `{pr_number}` PR description via REST — write the body to a file, split `{target_repo}` into `{$owner}` / `{$repo}` (or derive from `origin` on `{target_path}`), and `gh api repos/{$owner}/{$repo}/pulls/{pr_number} -X PATCH -F body=@<file>`. If the PR cannot be found because `{pr_number}` does not exist, verify the PR number and check `gh` auth before retrying.
