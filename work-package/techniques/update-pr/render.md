...
5. Update the `{pr_number}` PR description via REST — write the body to a file and `gh api repos/{owner}/{repo}/pulls/{pr_number} -X PATCH -F body=@<file>`. If the PR cannot be found because `{pr_number}` does not exist, verify the PR number and check `gh` auth before retrying.
