...
5. Reconcile before replacing an existing review: compare the `{live_review_body}` the op read against `{review_summary}`. Every difference is one of two things, and they are settled differently — this run's own pending change, which the replacement publishes, or an edit made to the live review out of band, which is adopted into `{review_summary}` and the artifact before the replacement goes out. The review API offers no concurrency token, so nothing detects the second case except this comparison, and a replacement made without it reverts the edit silently.
