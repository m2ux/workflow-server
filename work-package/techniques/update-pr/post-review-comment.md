...
4. Apply [post-pr-review](../../../meta/techniques/github-cli-protocol/post-pr-review.md) with `body` `{review_summary}` and `review_event` `{$review_event}`; set `{review_posted}` from the op. This is a pull-request review, not a description body update ([render](./render.md)).
