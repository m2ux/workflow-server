...
- engineering-link-mandatory: Engineering link is present, resolved from the remote and current branch of the checkout that holds the planning folder — never from the host repo's when that folder is a checkout of its own — and resolves to a committed file on the remote. [render](./render.md) step 3 owns that resolution.
