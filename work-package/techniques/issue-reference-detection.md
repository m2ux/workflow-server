...
2. When no reference is found, set `issue_present` to false and leave `issue_platform` and `issue_number` unset; downstream the `issue-verification` checkpoint handles the missing-issue case.
