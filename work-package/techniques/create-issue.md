...
- If a `gh api` command fails (auth, permissions, or network — including the issue verification in step 1), verify `gh` auth status and repository access, then retry or prompt the user to create the issue manually.
