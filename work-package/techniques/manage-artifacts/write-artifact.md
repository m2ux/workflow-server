...
1. Look in {planning-folder} for an existing instance of this artifact — a file matching `<NN>-{bare_filename}` (any two-digit prefix) or a bare `{bare_filename}`.
...
3. **If no instance exists → CREATE** `{artifact_prefix}-{bare_filename}` (e.g. `09-code-review.md`) under {planning-folder} and write `{content}` into it. The creating activity's prefix becomes the artifact's permanent number.
