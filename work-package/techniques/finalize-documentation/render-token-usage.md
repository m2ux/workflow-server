...
Token-use and cost-estimate artifact from the rolled-up session usage record, with a one-line README link.
...
2. Create `{token_usage_document}` at `{planning_folder_path}` per [artifact-prefix](../manage-artifacts/TECHNIQUE.md#artifact-prefix) containing:
