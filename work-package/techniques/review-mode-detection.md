...
2. When intent is clear, set `is_review_mode` accordingly and set `review_mode_ambiguous` to `false`. When the signal is ambiguous, set `review_mode_ambiguous` to `true` and leave `is_review_mode` unset.
