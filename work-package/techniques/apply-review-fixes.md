...
Implement the code-review findings and/or test improvements selected by the user, then commit the resulting changes. The commit is this technique's final phase — the fixes and their commit form one atomic step, so the calling activity needs no separate commit step.
...
- From `{findings}`, gather the code-review findings selected for fixing when `{needs_code_fixes}` is true, and the test-suite findings selected when `{needs_test_improvements}` is true.
...
- Apply [manage-git](./manage-git/TECHNIQUE.md)::[artifact-commits](./manage-git/artifact-commits.md) to stage and commit the fix changes on `{branch_name}`. This is the technique's final phase; the calling activity adds no separate commit step.
