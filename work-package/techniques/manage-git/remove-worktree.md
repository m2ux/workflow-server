...
2. Run `git -C {$component-git-dir} worktree remove {target-path}`. If the worktree has uncommitted edits, the command fails — surface this to the user rather than passing `--force`, since they may have intentional unfinished work; only remove after their explicit confirmation.
