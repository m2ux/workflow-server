...
Materialise a working directory as a git worktree of the component at `{target_path}` on `{branch_name}` — either creating a NEW feature branch off the component's default branch, or checking out an EXISTING branch (e.g. a PR's branch in review mode). The `{create_branch}` input selects between the two. Creates `{target_path}` and positions the branch in one step.
## Inputs
...
- Materialise path and branch position in one step by `{create_branch}`:  - When `{create_branch}` is true: `git -C {component_git_dir} worktree add -b {branch_name} {target_path} origin/{default_branch}`. If `{branch_name}` already exists on the component repo, ask the user whether to use the existing branch or pick a new name.
