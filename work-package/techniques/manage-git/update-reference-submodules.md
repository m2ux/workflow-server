...
- Run only when `reference-path` is set AND points at a monorepo (i.e. `{reference_path}/.gitmodules` exists). Skip silently when `reference-path` is empty or the reference is a standalone repo with no submodules.
