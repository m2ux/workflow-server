...
1. Identify the component git directory `{$component_git_dir}`, the same way [create-worktree](./create-worktree.md) does: `{repo_root}/{component_name}` when the repo root is a monorepo, `{repo_root}` otherwise. The worktree at `{target_path}` does not have to exist yet — any checkout of the component repo carries the same remote.
