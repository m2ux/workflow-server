...
1. Identify the component git directory and bind it to `{$component_git_dir}`, the same way [create-worktree](./create-worktree.md) does: `{reference_path}/{component_name}` when the reference is a monorepo, `{reference_path}` otherwise. The worktree at {target_path} does not have to exist yet — any checkout of the component repo carries the same remote.
