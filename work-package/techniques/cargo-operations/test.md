...
1. If cargo nextest is configured (.config/nextest.toml present in the project): `nice -n 19 SKIP_WASM_BUILD=1 CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-4} cargo nextest run {scope} {features} --test-threads=${RUST_TEST_THREADS:-4} {test-filter}`
2. Otherwise: `nice -n 19 SKIP_WASM_BUILD=1 CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-4} cargo test {scope} {features} {test-filter} -- --test-threads=${RUST_TEST_THREADS:-4}`
...
3. Report the run's pass/fail summary as `test-status`; when any test failed, capture the per-test detail as `failures`. If one or more tests failed, investigate the reported failure; do not mask it via --no-fail-fast.
