...
3. Compose {validation-results} = { {check-status}, {clippy-status}, {test-status}, {fmt-status}, {validation-passed}: {check-status}.passed AND {clippy-status}.passed AND {test-status}.passed AND {fmt-status}.passed }.