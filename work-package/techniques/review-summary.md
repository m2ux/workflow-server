...
- Resolve `{$reviewed_code_base}`: `{reviewed_code_base_url}` when it is non-empty; otherwise compose it from `gh pr view {pr_number} --json headRefOid,headRepository,headRepositoryOwner`.
